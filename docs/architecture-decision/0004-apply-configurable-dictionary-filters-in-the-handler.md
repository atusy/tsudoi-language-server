...
Dictionary completion originally hard-coded case-insensitive prefix matching into candidate
retrieval. Config authors need to replace that rule with LSP-side pipelines that can drop, reorder,
or rewrite candidates while keeping prefix matching as the default.
...
2. Apply arbitrary filters after a bounded storage query.
3. Apply iterable filter functions in the handler, using the index only for semantics-preserving
   narrowing.
...
apply the response bound only when no later stage can reorder, rewrite, or drop those entries.
Otherwise all entries in the current snapshot are passed to the handler pipeline.
...
This filter contract remains active after [ADR 0005](0005-build-immutable-dictionary-snapshots-in-memory.md)
replaced the storage and publication mechanism chosen by ADR 0002.
