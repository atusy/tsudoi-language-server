  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        {
          criterion:
            "The block opens with LABELLED FACTS, one per line, joined by a single newline and with NO rule between them: `source: <name>`, then for a FILE `size: <n> bytes` and `lastModified: <iso>`, and for a DIRECTORY `lastModified: <iso>` and NO `size:` LINE AT ALL. Asserted whole-value in all four cells -- {file, directory} x {markdown, plaintext} -- because the composer can read both axes and a relation asserted over one value of either is a green about that value. Falsified by a composer emitting `size:` for a directory, and by one still joining the facts with the rule.",
          verification:
            "bun test packages/tsudoi-completion-path/test/resolve.test.ts -- the four whole-value block arms, then PERTURBED: `statLine`'s directory branch made to emit a size line must redden `a directory's facts carry no size, where a file's carries one` AT ITS `size:` ASSERTION. The arms it ALSO reddens are recorded by name in test/perturbations.test.ts, as sprint 82's first record does; a reading whose only red is a collateral whole-value arm does not meet this criterion, which is the confounded-instrument failure PBI-82's criterion 6 shipped with and had amended at closing.",
        },
        {
          criterion:
            "The listing is announced by a HEADING carrying the total in every spelling, and announcing truncation only when it happens: `Entries (67)` when everything is shown, `Entries (first 20 of 67)` when it is not, `Entries (0)` for a directory holding nothing. A FILE's block carries no heading and no names. Falsified by a composer spelling `first 67 of 67` at the boundary, by one omitting the heading for the empty directory -- which makes `this directory holds nothing` and `no listing at all` the same bytes -- and by one dropping the total from the truncated spelling.",
          verification:
            'bun test -t "a directory at or under the bound shows every entry, and an empty one says so" and the bound arms beside it, each re-sited onto the new heading and each staging its own edge from the count it just read rather than spelling the bound; the FILE pair in the same arm is what makes `Entries (0)` a statement about a directory rather than the shape every answer has. Then PERTURBED: the comparison choosing the truncated spelling made inclusive must redden the AT-THE-BOUND arm and not the over-bound one.',
        },
        {
          criterion:
            "The two formats still produce DIFFERENT BYTES where markdown syntax exists and IDENTICAL BYTES where it does not, and BOTH halves are asserted: a resolved DIRECTORY's block differs between markdown and plaintext at the heading marker and the bullets, and a resolved FILE's block is byte-identical in the two, discriminated by `kind` alone. Falsified by a composer emitting a bullet or a heading marker into plaintext, and equally by an arm that asserts the difference for one kind and leaves the identity for the other unstated -- which is how the migrated arm went vacuous once already.",
          verification:
            'bun test -t "the markup a block is built in follows the session, not the item" widened to state both halves, THEN perturbed: the bullet prefix applied unconditionally must redden AT THE PLAINTEXT DIRECTORY VALUE, and the file-identity assertion must be the line that fails the day the file block gains markdown syntax. A run in which only the markdown arm moves does not meet this.',
        },
        {
          criterion:
            "The PREFIX relation survives the reshaping in all four cells: what the completion sent is a strict prefix of what resolve answers, for {file, directory} x {markdown, plaintext}. Falsified by inserting any fact before `source:`, and by a reorder applied to ONE format or ONE kind only -- which is the exact hole two independent reviewers found in sprint 82's prefix arm, one axis apart.",
          verification:
            'bun test -t "what completion sent is a strict prefix of what resolve answers, for both kinds", swept over both formats, THEN perturbed TWICE: the size line emitted before `source:` for MARKDOWN ONLY must redden it, and the same reorder for the `workspace` source only must redden it. Each perturbation names the cell its red lands in; a format-agnostic reorder alone does not meet this criterion, because it cannot see the difference it is being run to find.',
        },
        {
          criterion:
            "`lastModified` is truncated to the SECOND IN THE COMPOSER: an item whose file's real mtime carries a non-zero millisecond renders a value with no fractional part. Falsified by truncating in the fixture instead, which leaves every test green and every real file's popup carrying the milliseconds.",
          verification:
            "A new arm in packages/tsudoi-completion-path/test/resolve.test.ts staging a file stamped at a non-zero millisecond, which ASSERTS ITS OWN PREMISE FIRST by reading the stamp back -- a filesystem that truncates sub-second precision itself would make the arm measure nothing and it must say so rather than pass. Then perturbed: the raw ISO string restored must redden THAT arm at its `lastModified` assertion, and the whole-value arms it also reddens are recorded by name.",
        },
        {
          criterion:
            "A stat-driven answer is still told from a `kind`-driven one BY THE ITEM'S OWN BYTES, now that the words `file` and `directory` have left the block. The discriminators are `size:` PRESENT AGAINST ABSENT and the entries heading, and BOTH the present case and the absent case are asserted -- so a handler emitting NEITHER line fails rather than satisfying an absence. Falsified by an answer composed from `item.kind` instead of from the stat, in either direction.",
          verification:
            'bun test -t "a file whose item claims to be a folder is still answered as a file" and bun test -t "a directory whose item claims to be a file still comes back with its listing", re-sited. The reds must land in DIFFERENT PLACES and that is why they stay two arms: the file arm at its `size:` PRESENT assertion, the directory arm at its heading. THEN perturbed with a composer emitting neither a size line nor a heading, which must redden the file arm at the present assertion -- an arm resting only on absence stays green under it.',
        },
        {
          criterion:
            "Both `listingSection` helpers locate the listing under the NEW heading, and still find the real one when an entry's own name imitates it: a directory holding an entry named like the heading renders a names part matching that pattern, and the header is always the earlier match. Falsified by an unanchored or last-match lookup, and by the two copies disagreeing -- the docblock says they must not.",
          verification:
            "The sprint-82 anchor re-run in its new form: an arm handing each helper a block composed under the new heading must redden BEFORE the helpers are re-derived and pass after, and the imitating-entry arm must redden under an unanchored lookup. Both copies -- packages/tsudoi-completion-path/test/resolve.test.ts and test/resolve-path-stat.test.ts -- move in the same commit; the witness lives in the member half, since the root half resolves only items a real server produced.",
        },
      ],
      status: "ready",
      notes: [
        "THE STAKEHOLDER'S BLOCK, QUOTED RATHER THAN PARAPHRASED: `source: cwd` / `size: 1062 bytes` / `lastModified: 2024-06-19T12:00:00Z` / blank / a level-one heading reading `Entries (first 20 of 67)` / blank / a bulleted name. THREE THINGS CHANGE AT ONCE -- the facts become LABELLED and are separated by ONE newline rather than by a rule, the stat's three facts stop being one dot-separated sentence, and the listing gains a HEADING. THE RULE GOES ENTIRELY: there is no thematic break anywhere in the target block, in either format, which is the fact criterion 3 exists to keep honest.",
        "THE KIND IS RULED BY THE STAKEHOLDER AND THE RULING COSTS SOMETHING THIS ITEM MUST PAY. Asked directly with the two blocks side by side, they chose the example as written: no `kind:` line, a directory legible by carrying no `size:`. So the word `file`/`directory` LEAVES, and the two arms sprint 82 landed to tell a stat-driven answer from a `kind`-driven one lose the byte they read. THE OBJECTION WAS PUT AND OVERRULED, recorded because the ruling is theirs and the consequence is ours to carry rather than to re-raise. Criterion 6 is that consequence, and its PRESENT-CASE PAIR is the part a reader will be tempted to drop: an arm resting on absence alone is satisfied by a handler that emitted nothing.",
        "`lastModified` IS TRUNCATED TO THE SECOND, RULED BY THE STAKEHOLDER. `Date.prototype.toISOString` always emits milliseconds, so this is a composition step and not a spelling. IT COSTS NOTHING THE FIXTURES RELY ON -- both stamp constants are already whole seconds, deliberately, because filesystems disagree about sub-second precision -- AND THAT IS EXACTLY WHY THE CRITERION NEEDS A FIXTURE THAT IS NOT: against whole-second stamps, a composer that truncates and one that does not are told apart by a trailing zero fraction, which is a weaker reading than the one that matters, and a truncation placed in the fixture would satisfy every existing arm.",
        "THE PLAINTEXT SPELLING, RULED BY THE PRODUCT OWNER: plaintext is the markdown block WITH THE MARKDOWN SYNTAX REMOVED AND NOTHING ELSE CHANGED -- no heading marker, no bullet before each name. The labels and the heading TEXT are identical in both, because a label is content and not syntax, and a client that renders nothing must still be able to find a fact by its name. AND THE CONSEQUENCE IS DISSENT-WORTHY, SO IT IS STATED RATHER THAN LEFT TO BE DISCOVERED: with the rule gone, a resolved FILE's block holds no markdown syntax at all, so its two formats become BYTE-IDENTICAL and only `kind` discriminates. That is the vacuity sprint 82 migrated this very arm to escape, arriving one level up in the sprint after. IT IS ACCEPTED RATHER THAN FOUGHT -- the alternative is inserting bytes the stakeholder's block does not have, purely so a test has something to read -- and criterion 3 answers it the way sprint 82 answered the same thing: the identity is ASSERTED, so the day a file's block gains syntax, the line that stops being true is in the file.",
        "THE HEADING'S THREE SPELLINGS, RULED BY THE PRODUCT OWNER: `Entries (67)` when nothing is truncated, `Entries (first 20 of 67)` when something is, `Entries (0)` when the directory is empty. THE TOTAL IS IN EVERY SPELLING AND IN THE SAME PLACE -- last number in the parentheses -- and truncation is announced only when it happens, which is the switch today's sentence already makes. `Entries (0)` RATHER THAN AN OMITTED HEADING is the load-bearing half: sprint 82's arm pairs the empty directory with a FILE precisely because, with names alone, `this directory holds nothing` and `nothing was listed` produce the same bytes. A READER MAY REASONABLY PREFER ONE UNIFORM TRUNCATED SPELLING: it would give the helpers a single pattern instead of an alternation. Refused because `first 67 of 67` is what the item's own note calls badly-reading, and the alternation is one branch in a lookup that is being rewritten anyway.",
        "AND ONE THING OBSERVED AND DELIBERATELY NOT CHANGED: a level-one heading renders very large inside a hover popup on some clients. It is the stakeholder's byte and this item keeps it. If it renders badly it is the same class of question as PBI-82's standing inline-truncation dissent -- a rendering fact this repository cannot measure -- and it is reopened as its own item rather than argued here.",
        "WHAT THIS MUST NOT BREAK, EACH LANDED BY SPRINT 82 AND EACH CHEAP TO BREAK HERE. The PREFIX relation, which survives a one-newline join and dies if anything is inserted before `source:` -- criterion 4. The DIRECTORY-CARRIES-NO-BYTE-COUNT ruling, which `size:` makes easier to violate by making the shape uniform -- criterion 1. And BOTH `listingSection` helpers, which now locate the listing by its header and so move when the heading does -- criterion 7, whose anchor is the sprint-82 finding that a wrong lookup goes GREEN.",
        "THE TWO-AXIS SWEEP IS A CONDITION ON EVERY BLOCK-SHAPE CRITERION HERE AND NOT A STYLE NOTE. Sprint 82's largest finding, now an active improvement: a relation asserted over ONE value of a discriminator the composer can read is a green about that value. This item moves the composer on the FORMAT axis and the KIND axis at once, and the two reviewers who found that hole found it one axis apart. So criteria 1, 3 and 4 are each swept over both, and criterion 4's perturbation is deliberately applied to one cell at a time -- a format-agnostic reorder cannot see the difference it is run to find.",
        "AND EVERY PERTURBATION HERE NAMES WHERE ITS RED MUST LAND, WHICH IS THE OTHER SPRINT-82 CARRY-FORWARD. PBI-82's criterion 6 shipped naming a perturbation that reddened its arm at a MEMBERSHIP assertion predating the sprint -- a red that reads as proof -- and had to be amended at closing. In this file the composer is compared whole almost everywhere, so nearly any edit reddens nearly everything; a criterion that asks only for a red asks for nothing.",
      ],
    },
    {
      id: "PBI-84",
