    ...
    subtasks: [
      {
        ...
        notes: [
          ...
          "PREDICTIONS AND COUNTERFACTUALS WERE WRITTEN BEFORE THE RUN and all eight held: the null narrower than the shape deltas at the ordinary size; deno outside the null there; on bun readdir+sort SLOWER and readdir+retain FASTER than streaming at 5000; the ranking on bun FLIPPING WITH SIZE (readdir+sort faster at 200, slower at 5000 and at the tail), which is why one number per runtime could never have settled this; deno's open alone between 1 and 5 ms; no cell arriving in render order; the tail's ordering; and the Definition of Done unmoved by the instrument. NOTHING WAS FITTED AFTERWARDS -- the one thing the run contradicted is the plan's two arrival orders, above, and it is recorded as a contradiction rather than dropped.",
          "WHAT THIS INSTRUMENT CANNOT SEPARATE, and it is the fourth term a MEASURED label owes: one machine, one volume, a warm cache and empty files. It cannot tell a runtime's cost from this filesystem's, and it says nothing about a cold cache, a network mount or non-empty entries. It also carries a COPY of the package's gate and comparator -- named in its own docstring -- so it cannot catch the package's gate changing while the copy does not.",
      ...
      },
      {
        ...
        notes: [
          ...
          "THE THIRD SHAPE WAS FASTEST EVERYWHERE AND WAS NOT SELECTED, WHICH IS REPORTED RATHER THAN LEFT IMPLICIT. readdir+retain is quicker than the shipped shape at every size on both runtimes -- bun -0.031 / -0.601 / -12.277, deno -0.200 / -4.157 / -88.167 -- and the rule for this branch selects against the WORKING SET, not against the clock. It loses there for the reason above, and it would also delete the second cancellation seam. NO ARM WAS DELETED AND NO ROUTING WAS TRIGGERED: the routing binds when the reading SELECTS that shape, and the rule did not select it.",
      ...
      },
      {
        ...
        notes: [
          ...
          "THE PERCEPTION CLAIM WAS DROPPED RATHER THAN DRESSED. Neither honest neighbour was obtainable: no editor, plugin chain or setting was measured, and the delta's other neighbour -- a named budget with provenance -- is what the ruling stands on instead, which is that this runs ONCE PER HIGHLIGHT on a moment the user is already waiting through, never on the keystroke path the completion half owns. The arrowing amplifier is stated as a MACHINE cost at the per-highlight number and explicitly not as a latency, and no product of that number with a keystroke count is manufactured.",
          ...
          "FILED AS SUSPECT BY ASSOCIATION RATHER THAN RENUMBERED, each with the ruling it belongs to: the `~1.1 s` of per-entry stats, whose subject is the refusal this package exists for and which nothing in this reading measures -- multiplying the single-stat cell by an entry count would be arithmetic wearing a measurement's clothes; and the 777-859 ms durations at the abort seam, taken at a hundred thousand entries where this session re-took the open only at five thousand. The seam's argument rests on the TURN COUNT rather than on those durations, and that count was not re-taken either, which the site now says.",
