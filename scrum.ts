    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          ...
          "THE RATE, NOT THE REPAIRS. This sprint's prose kept making claims about its own text that the text did not support -- in the sprint whose subject is that class. CAUGHT BEFORE LANDING: the arm's first draft said DELETE, NARROW and SUPERSEDE all fire on a fact having CHANGED, which DELETE's own text refutes. LANDED IN THE ARM AS SHIPPED: `Everything above in this file is what a mutable present-tense home costs`, over a corpus whose entries include prose false on arrival -- found by a reviewer, F2. LANDED IN A REPAIR: `the five in test/published-artifacts.test.ts`, a count copied rather than taken, six on measurement -- found by me re-reading my own repair. LANDED IN A REPAIR: `an ORDERING over homes this section already carries`, wrong about the entries it inherited AND about the entries it added -- found by a reviewer, F11. LANDED IN F11'S OWN REPAIR, TWICE OVER, AND THIS ENTRY HAS ALREADY HAD TO BE CORRECTED ONCE: F11's repair said `restate` of both of THE COMMIT's triggers, which is imprecise about one of them; the repair of THAT, filed as F12, replaced it with `THE ONE ENTRY THIS PARAGRAPH CANNOT PLACE` and manufactured a conflict between two rules that never apply to the same case, because the list's entry condition partitions them. F13, by a reviewer. THIS ENTRY FIRST CLAIMED F12 AS A FINDING I FOUND MYSELF; it was a worse defect than the one it replaced, and the reviewer found it. MOST OF WHAT LANDED WAS INTRODUCED BY THE REPAIR PASS ITSELF, EACH REPAIR REVIEWED AS SOUND WHEN MADE, AND ONE SITE HAS NOW BEEN WRONG THREE TIMES RUNNING. NO TOTAL IS WRITTEN: the round is open, a total taken mid-round is the count this sprint has twice deleted, and this entry alone has moved twice.",
          "F12 WAS FILED HERE AS AN UNRESOLVED CONFLICT AND F13 REFUTED IT; THE RETRACTION STAYS BECAUSE THE FILING DID. It claimed `.claude/skills/writing-a-comment/SKILL.md` gives a FORECLOSED ALTERNATIVE two competing homes -- a line at the reintroduction site, and THE COMMIT. THE SCOPES PARTITION AND THERE IS NO CONFLICT: the list is entered only WHEN THE REASON IS NOT A COMMENT, so an alternative that earns its line at the site is a comment and never reaches the bullets, and one that does not is not a comment and goes to the commit. One condition, two branches. Note 1 of PBI-81 says the same in the stakeholder's words -- foreclosed alternatives mostly belong in the commit, in source only minimally and conditionally. WHAT THE FILING ACTUALLY DID was invoke the sprint-record arm to defer a question nobody had asked, which is the arm working on a false premise rather than the arm working.",
