...
const scrum: ScrumDashboard = {
    "product_goal": {
      "statement": "tsudoi lets developers assemble a Language Server for any language by writing a single TypeScript config file, without implementing the LSP protocol themselves.",
      "success_metrics": [
        {
        ...
        {
          "metric": "The five methods the stakeholder named respond per the specification: textDocument/completion, textDocument/hover, textDocument/diagnostic (pull), textDocument/formatting, completionItem/resolve",
          "target": "5 of 5. ENUMERATED IN THE METRIC ITSELF because `10 of 10` stood for thirty sprints with NOTHING ANYWHERE ENUMERATING THE TEN -- grepped, the only match was the metric. A fraction whose denominator nobody can name cannot be met, and the PO twice reported `2 of 10` as fact. The five were set by the stakeholder, not invented to make the metric satisfiable."
        },
        {
          "metric": "The CLI starts under both Bun and Deno",
          "target": "Smoke start succeeds on both runtimes, both from a repo checkout and from an installed package"
        }
      ]
    },
    "product_backlog": [
      {
        ...
        "story": {
          "role": "config author",
          "capability": "name tsudoi by ONE name -- in package.json, in every import specifier, and in the docs",
          "benefit": "one name to learn and to type, rather than a package whose name and whose repository disagree"
        },
        "status": "ready",
        "acceptance_criteria": [
          {
            "criterion": "`@atusy/tsudoi` IS GONE AS A PACKAGE NAME AND AS A SPECIFIER, AND `@atusy/tsudoi-language-server` RESOLVES ON EVERY ROUTE THE OLD NAME RESOLVED ON. Three routes, and all three are the criterion rather than one standing for the others: tsc through the `paths` mapping in tsconfig.json, the in-repo runtime through the exports map into dist/, and an installed consumer through the tarball.",
            "verification": "THE POSITIVE DIRECTION IS MEASURED AGAINST A CENSUS TAKEN AND COMMITTED BEFORE ANY FILE IS TOUCHED, and this is the whole of the verification rather than a refinement of it. A ZERO-RESULT GREP FOR THE OLD NAME IS EQUALLY TRUE OF A REPO WHERE THE SWEEP WORKED AND ONE WHERE SOMEONE DELETED THE FILES -- Sprint 39's ambiguity in its most literal form, and `absence` is not a thing a rename should ever be judged by alone. SO: BEFORE, a PER-FILE census of every occurrence of `@atusy/tsudoi`, committed. AFTER, EACH FILE IN THAT CENSUS STILL EXISTS and carries the SAME COUNT of the new name that it carried of the old. PER FILE AND NEVER IN TOTAL, because a total is met by one file gaining two while another loses two. A CENSUS FILE THAT NO LONGER EXISTS FAILS THIS CRITERION rather than counting as zero. The old-name grep is then the cheap confirmation it was always meant to be, with every remaining hit accounted for BY NAME -- the `completed` and `retrospectives` entries the third criterion protects are the expected ones. The four DoD checks pass, which is what exercises all three routes."
          },
          {
            "criterion": "THE README'S EXECUTED BYTES ARE THE SUBJECT AND NOT A DOCUMENT UPDATED ALONGSIDE. The quickstart's install command, both start command lines and the config's import line are extracted from this README and run, so the rename lands in them or the suite fails.",
            "verification": "The suite passes WITH NO CHANGE TO THE EXTRACTION MACHINERY. If test/helpers/readme.ts had to be edited, that is REPORTED rather than absorbed: it would mean the rename reached the instrument as well as its subject, and an instrument edited in the same commit as its subject cannot testify about it."
          },
          {
            "criterion": "THE RECORD IS NOT REWRITTEN. scrum.ts's `completed` sprints and `retrospectives` name `@atusy/tsudoi` throughout and were TRUE WHEN WRITTEN. They are left VERBATIM -- the same treatment Sprint 42's improvement outcome gave its own superseded action text.",
            "verification": "The rename's diff touches no entry under `completed` or `retrospectives`, and ONE note records that the old name is what those entries meant. A sweep that 'helpfully' updates them fails this criterion."
          }
        ],
        "notes": [
    ...
    "retrospectives": [
      ...
      },
      {
        ...
        "improvements": [
          {
            "action": "A COUNT CAN LIVE IN A FILENAME, AND A CONTENT GREP DOES NOT SEE ONE. Extension of the grep-the-claim's-words entry: search FILENAMES and TEST NAMES as well as file contents. Measured this sprint -- seven sites said `the eight` against a nine-name list, one in a test name and TWO IN PROBE FILENAMES.",
            "timing": "immediate",
            "status": "active",
            "outcome": null
          },
          {
            "action": "`NONE WEAKENED` IS DIFFED, NOT ASSERTED. It is a COVERAGE CLAIM and S13 forbids recalling one; diffing every `expect(` line across test/ and src/ is cheap and gives a direction, not an impression. MEASURED this sprint: seven added, zero removed, zero changed, and the pin's toEqual unchanged with only the object literal grown. The PO records having accepted that claim ON ASSERTION FOR A DOZEN SPRINTS.",
            "timing": "immediate",
            "status": "active",
            "outcome": null
          },
          {
            "action": "A COUNT THAT IS A MEASUREMENT RESULT CARRIES PROVENANCE AND IS NOT EDITED WITHOUT RE-MEASURING; A COUNT THAT IS A DESCRIPTION IS REPLACED BY NAMING. DIFFERENT OBJECTS THAT LOOK IDENTICAL IN PROSE, and the distinction bounds prefer-naming-to-counting so it is not over-applied to numbers that are evidence.",
            "timing": "immediate",
            "status": "active",
            "outcome": null
          }
      ...
      },
      {
        ...
        "improvements": [
          {
            "action": "AN EXECUTOR RE-MEASURES A NUMBER THEY WERE HANDED RATHER THAN COPYING IT. This has now caught a handed-down count in TWO CONSECUTIVE SPRINTS -- nine-not-eleven, then eleven-not-six -- and it works BECAUSE OUR RECORDS CARRY VERSION AND PATH, the S8 Sprint-24 amendment paying out. It generalises to a handback from anyone including the PO, and it reaches something no rule about the author can: A BRIEF IS THE ONE ARTIFACT WITH NO PERMANENT HOME, so an error in it is caught by the recipient re-measuring or not at all.",
            "timing": "immediate",
            "status": "active",
            "outcome": null
          }
      ...
      },
      {
        ...
        "improvements": [
          {
            "action": "A HAZARD MUST OWN A TEST WHOSE FIRST ASSERTION IT IS. Two hazards sharing one test means the second can never be OBSERVED: the same perturbation flips the first and the test stops there.",
            "timing": "immediate",
            "status": "active",
            "outcome": "Found at Sprint 18, where a read-time fallback broke TWO ways -- the first delta replacing the root, and a later removal making the root REAPPEAR -- and the second was visible only because it owned its own test. A PRECONDITION FOR THE PERTURBATION-DISCIPLINE RULE RATHER THAN ITS MIRROR, which is why it is an entry rather than a clause: in the bundled counterfactual the perturbation flips AT the headline rather than earlier, so `flips earlier than the headline` never fires -- and in a two-claim test `the headline` has no single referent, so that rule cannot be applied reliably at all. Different trigger, different actor, different moment: the TEST AUTHOR before any perturbation is run, not the perturbation runner interpreting a flip."
          }
        ]
      },
      {
      ...
      },
      {
        ...
        "improvements": [
          {
            "action": "When a perturbation CANNOT BE CONSTRUCTED, classify it. NOT CONSTRUCTED: the means were lacking -- the assertion is undefended, say what remains at risk.",
            "timing": "immediate",
            "status": "active",
            "outcome": "Filed by the Developer against themselves: their vocabulary had three outcomes -- reddened, did not redden, could not build it -- and the third defaulted to the pessimistic reading, so they reported a DESIGN SUCCESS in the language of a coverage gap."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            ...
            "timing": "sprint",
            "status": "active",
            "outcome": "A bounding condition on seven sprints of pin-everything pressure, whose cost is already visible: PBI-9 carries three separate instances of hardcoded-response-id brittleness -- tests that resist legitimate change without defending a requirement."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            "action": "Every assertion that something is ABSENT -- zero stderr, zero $/progress, a label not on stdout -- ships with a PAIRED assertion, permanent in the suite, that the same measurement observes it when present.",
            "timing": "immediate",
            "status": "active",
            "outcome": "Generalises what the PO had been imposing by hand criterion by criterion."
          },
          {
            "action": "A perturbation specified by the PRODUCT OWNER names the assertion it is required to flip, not just the mutation to make.",
            "timing": "sprint",
            "status": "active",
            "outcome": "Filed separately from the perturbation-LABELLING rule on purpose: that one governs how the Scrum Master REPORTS (reproduction versus independent, expected versus observed), this one governs how the PO AUTHORS."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            "action": "A helper that terminates a subprocess must settle every promise it owns before the process dies. Cross-test misattribution is a suite-integrity failure, not a single-test bug.",
            "timing": "immediate",
            "status": "active",
            "outcome": "Fixed in Sprint 5; recorded so the shape is recognisable if PBI-9's helper work recurs it."
          },
          {
            "action": "Standing item 6, AMENDED at Sprint 13 (no exception carved -- exceptions rot): the stakeholder-facing example is EXECUTED by the suite -- the config is loaded and driven, and a change that breaks it must redden a named assertion. It need NOT be the config that carries every property assertion; purpose-built configs may.",
            "timing": "immediate",
            "status": "active",
            "outcome": "TWO negative controls, named separately because they are different failures: breaking its IMPORT must redden (the Sprint 9 case), and breaking a HANDLER'S RETURN must redden. Supersedes the with-no-fixture-copy-in-existence wording, which forbade purpose-built fixtures rather than forbidding an unexecuted example."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            "action": "PERTURBATION DISCIPLINE, one rule: anything not perturbed is assumed UNPROVEN; every subtask declares expected-RED or born-green; every perturbation is named by the ASSERTION it flips, not by the subtask it belongs to. If it flips at an EARLIER assertion than the subtask's headline claim, PREFER SPLITTING OVER DOCUMENTING -- the earlier flip is a signal that the test BUNDLES independent sub-claims, and splitting DISSOLVES what a note would only describe.",
            "timing": "immediate",
            "status": "active",
            "outcome": "MERGED AT SPRINT 14 from two statements of one rule, nothing dropped, and the second was always a corollary of the first's last clause. The base rule had ALREADY been amended three times, which is its own signal: a rule list nobody can hold in their head stops being applied at exactly the moment it is needed -- which is the argument for merging rather than against it."
          },
          {
            "action": "The PO's Review checklist splits into a STANDING list, recorded here once and reported against at EVERY Review, plus a short per-sprint list of what is genuinely new.",
            "timing": "immediate",
            "status": "active",
            "outcome": "Nine items where three carried new information diluted the signal the item-by-item rule exists to protect. MERGED AT SPRINT 17 from the S1 timing rule, one rule about one artifact: the checklist is ISSUED AT PLANNING rather than at Review, so the plan can target it."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            "action": "A Review perturbation states whether it REPRODUCES the Developer's recorded perturbation or is INDEPENDENT.",
            "timing": "sprint",
            "status": "active",
            "outcome": "Prompted by a Review perturbation reddening 6 tests where the Developer's reddened 2."
          }
        ]
      },
      {
        ...
        "improvements": [
          {
            "action": "When a planning spike produces passing code, ATTACH it for the executor to start from -- the plan then says what to change about it instead of re-deriving it in prose -- and the attachment must be DURABLE: inlined verbatim in the subtask text, or committed into the repo by the first subtask.",
            "timing": "sprint",
            "status": "active",
            "outcome": "MERGED AT SPRINT 13 from the S1 attach rule and the S2 durability rule, nothing dropped."
          }
        ]
      }
    ]
