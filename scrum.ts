  ...
  product_backlog: [
    {
      ...
      story: {
        role: "config author",
        capability:
          "answer `textDocument/codeAction` from a handler key -- a NEW ROW of the request table `textDocument/hover` and the rest are rows of, with the same lifecycle gate, the same cancellation and the same params refusal, and none of it written again",
        benefit:
      ...
      notes: [
        ...
        "THIS ROW LEAKS `any` ONTO AN AUTHOR'S SURFACE THROUGH `Diagnostic.data`, reached at `CodeActionParams.context.diagnostics` -- AND THE FIELD FIRST WRITTEN HERE WAS `CodeAction.data`, WHICH IS THE WRONG HALF AND WORTH THE CORRECTION RATHER THAN A QUIET EDIT. `CodeAction.data` is RESULT-side: the author writes it, and `any` in a position you write disables checking on nothing you received. It is inert here besides, this row advertising a boolean provider that cannot declare `resolveProvider`, so nothing ever hands it back. The leak is PARAMS-side, the same direction as `CompletionItem.data` and `ExecuteCommandParams.arguments`. PRE-EXISTING AND REFUSED HERE FOR SPRINT 86'S REASON: reshaping what upstream declares a request CARRIES would be taken for every row at once or not at all, and doing it for one row would have tsudoi publish a second name for a type upstream owns -- which is what the `deps/` split exists to prevent.",
