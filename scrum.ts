  ...
  product_backlog: [
    {
      ...
      notes: [
        ...
        "THIS ITEM'S OWN SHAPE IN THE READER UNDERNEATH THE INSTRUMENT BUILT FOR IT, FILED INTO PBI-70 BY NAME. Raised UNMEASURED by the product owner, who had no shell and said so; MEASURED here because it was cheap, and it is as filed. `readReport` in `test/helpers/perturbation.ts` maps a THREE-VALUED reality onto two: MEASURED on bun 1.3.13, `--reporter=junit` writes a skipped testcase as a `<testcase>` carrying a `<skipped />` child and NO `<failure`, in all three spellings -- `test.skip`, `test.todo` and `test.if(false)` -- and the reader, which decides `failed` by the presence of `<failure` and `passed` by everything else, records PASSED for each of them. SO A RECORD WHOSE NAMED ARM HAS BEEN SKIPPED READS `GONE QUIET`, sending its author to repair an arm that is fine, where the honest reading is the one this module spends a whole arm defending: REFUSED, because the arm did not run. THE PAIRED ASSERTION DOES NOT CATCH IT EITHER, which is the half worth having measured: `every arm in test/definition-of-done.test.ts passes before any weakening` requires an empty failure list BESIDE a non-empty report, and a file whose arms are all skipped satisfies both. IT IS THE REFUSED-VERSUS-GONE-QUIET COLLAPSE ONE LAYER DOWN from the arm that defends it, which is why it is this item's own class rather than a reader bug.",
      ],
    },
