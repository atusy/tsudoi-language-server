  ...
  product_backlog: [
    ...
    },
    {
      ...
      acceptance_criteria: [
        {
          criterion:
            "THE TYPE CHECK CANNOT READ A STALE `dist/`. examples/ import `@atusy/tsudoi/types`, which package.json maps to `./dist/types.d.ts` -- THE BUILT ARTIFACT -- so `tsc --noEmit` checks them against whatever dist/ happens to hold, while `bun test`'s preload REBUILDS it first. The two therefore disagree EXACTLY WHEN THE PUBLISHED SURFACE HAS MOVED, which is the only time either matters.",
          verification:
            "TO BE AUTHORED AT REFINEMENT. THE PREMISE IS ALREADY MEASURED THREE TIMES AND IN BOTH DIRECTIONS, so refinement does not re-establish it: Sprint 42 saw `tsc --noEmit` EXIT 0 beside 43 test failures, and EXIT 1 on a CLEAN src/ against a leftover dist/; Sprint 43 saw it manufacture a TS2322 naming a TUPLE TYPE THE TREE NO LONGER CONTAINED. IT LIES IN BOTH DIRECTIONS. WHAT REFINEMENT MUST DECIDE is where the fix lives -- bunfig.toml's preload protects the suite and NOTHING protects the type check.",
        },
      ],
      notes: [
