    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          ...
          "THE RATE, NOT THE REPAIRS. This sprint's own prose produced FOUR claims about its own text that the text did not support, in the sprint whose subject is that class. ONE was caught before landing: the arm's first draft said DELETE, NARROW and SUPERSEDE all fire on a fact having CHANGED. THREE LANDED. In the arm as shipped, `Everything above in this file is what a mutable present-tense home costs`, over a corpus whose entries include prose false on arrival -- found by a reviewer, repaired at F2. IN A REPAIR, `the five in test/published-artifacts.test.ts`, a count copied and not taken, six on measurement -- found by me while re-reading the repair. IN A REPAIR, `an ORDERING over homes this section already carries`, false about the two entries it inherited and about the two it added -- found by a reviewer, F11. TWO OF THE THREE THAT LANDED WERE INTRODUCED BY THE REPAIR PASS, and both repairs had been reviewed as sound when made.",
