  ...
  product_backlog: [
    ...
    },
    {
      ...
      acceptance_criteria: [
        {
          criterion:
            "THE KIND IS DECLARED ON THE ENTRY AND BOTH KINDS REACH A HANDLER. `textDocument/didFocus` declared as a request is answered; declared as a notification its handler runs and NOTHING is written back. THE DEFECT THIS PREVENTS IS INFERENCE FROM THE NAME: upstream has already dispatched by the time tsudoi sees a message and hands `onRequest` only `(params, cancellation)`, so a custom name carries no kind and registration is a compile-time choice between `onRequest(method: string, ...)` and `onNotification(method: string, ...)`.",
          verification:
            "Fixture configs spawned as real servers under the fake editor, one custom entry each. PROBE, BOTH KINDS: the request entry is answered; the notification entry runs its handler -- observed through a LATER request reading what the handler wrote, a notification having no response to read -- and nothing is written back for the notification itself. NEGATIVE CONTROL, AND NOT THE ONE THAT LOOKS OBVIOUS: flipping the declared kind leaves the request unanswered, but so does a name registered NOWHERE -- upstream answers MethodNotFound in both -- so that red grades nothing alone. The control is THE PAIR MOVING TOGETHER: with the kind flipped, the request goes unanswered AND the same name delivered as a notification now reaches the handler, read against a third arm on a name absent from the map where neither form reaches anything.",
        },
        {
          criterion:
            "`customMethodHandler` RESOLVES BOTH THE CONTEXT AND THE RETURN FROM THE KIND, AND NEITHER BASE CONTEXT NAME IS PUBLISHED. Request resolves to `BaseRequestContext` and `Promise<{ result: unknown }>`, notification to the session-only supertype and `Promise<void>`, and a handler of either kind compiles INLINE with the author naming no context type -- which is the discipline `RequestContext`'s docblock already states as `nothing an author writes selects the shape they are handed`. TWO MEASURED MISTAKES LIVE HERE AND BOTH COMPILE SILENTLY: a conditional written on the CONTEXT TYPE takes the notification arm for both kinds, the notification context being the SUPERTYPE, and the arms have been drawn INVERTED with nothing objecting.",
          verification:
            "A consumer-shaped type probe in the register of test/initialize-handler-types.test.ts, grading each arm by IDENTITY and not by assignability -- mutual assignability is exactly what makes both mistakes silent. NEGATIVE CONTROLS, TWO, EACH GREEN TODAY: the arms swapped must redden, and the conditional rewritten on the context type must redden. AND THE SURFACE ARM, TAKEN THROUGH THE PUBLISHED `./types` ARTIFACT AND NOT THROUGH `src/`: no exported name for the notification context.",
        },
        {
          criterion:
            "`{ result: null }` REACHES THE CLIENT AS A NULL RESULT, AND A REQUEST HANDLER THAT ANSWERS NOTHING IS A HANDLER FAILURE. A typed row says `Hover | null` and distinguishes the two by its own type; a custom result is `unknown` and cannot, which is what the wrapper is for -- and a wrapper whose missing case silently becomes null hands back the very conflation it was added to remove. PRODUCT OWNER RULING ON THE MISSING CASE, and it applies code that exists rather than inventing a semantic: it takes the route a THROWING handler already takes -- `reportHandlerFailure` names the method on stderr and rethrows, and upstream turns the throw into an error response. REVERSAL CONDITION: the day an author reports that falling off the end should be a silent no-op, this is what to revisit and not the wrapper.",
          verification:
            "Fake-editor arms reading the RAW response: a handler returning `{ result: null }` yields a response carrying a null result and no error; one returning nothing yields an error response and a `tsudoi: ` line naming the method. NEGATIVE CONTROL: an implementation that sends the handler's return bare must make those two arms indistinguishable on the wire.",
        },
        {
          criterion:
            "A NOTIFICATION HANDLER THAT ANSWERED, AND ONE THAT REJECTED, ARE EACH NAMED ON STDERR ONCE PER METHOD PER SESSION, WITH STDOUT UNTOUCHED. THIS IS THE ONLY ENFORCEMENT THERE IS, which is what makes it worth the code: src/config.ts reaches the config through a cast from `unknown`, so `Promise<void>` constrains a real config nowhere and an author gets no diagnostic from anything today. PER METHOD AND NOT PER SESSION, which is the spelling that looks right and loses a diagnostic: one flag for the session means the first report on any custom method permanently silences every other. AND THE REPORTING PATHS ARE PRICED RATHER THAN COPIED: `reportInvalidToken` takes a `Method` and `handlerFailure` a `ConfigMethod`, while a custom name is a bare string, so every signature this criterion leans on widens.",
          verification:
            "One fixture carrying two custom notifications, one answering and one rejecting, each driven repeatedly. ARMS: exactly one `tsudoi: ` line per method, naming it; the session still answers a request afterwards; and no non-LSP byte on stdout. NEGATIVE CONTROLS, TWO: a single session-wide flag must lose the SECOND method's line; and with the report removed, a rejecting handler is expected to be observable by NOTHING -- tsudoi awaits the handler, so the rejection is caught by the awaiting frame rather than reaching either runtime's unhandled-rejection path. TAKE THAT READING BEFORE THE ARM IS BELIEVED: if the silence is what it measures, this criterion is the only signal there is, and the arm grades stderr rather than an exit code.",
        },
        {
          criterion:
            "A CUSTOM NOTIFICATION ENTRY DECLARING NO GATE DOES NOT TYPE-CHECK, AND A CUSTOM REQUEST ENTRY IS NOT ASKED FOR ONE. `NotificationEntry.gate` is required with no default so that an entry deciding nothing is a compile error rather than a handler silently running in every lifecycle state; a gate defaulted on the config side re-creates exactly the convention that docblock refused, one door along and for the author instead of the maintainer.",
          verification:
            "Compile probe, BOTH DIRECTIONS: a notification entry without a gate is refused naming the field; a request entry without one compiles. RUNTIME ARM: a lifecycle-gated custom notification arriving before `initialize` does not reach its handler and an always-gated one does, both read through a later request. NEGATIVE CONTROL: a defaulted gate must make the first probe pass.",
        },
        {
          criterion:
            "A CUSTOM NAME COLLIDING WITH THE REQUEST TABLE IS REFUSED BY A MESSAGE NAMING THE METHOD AND THE RULE, AND A BUILT-IN NOTIFICATION NAME IS NOT REFUSED. MEASURED TODAY: the refusal arrives as TS2322 `not assignable to type 'undefined'`, which names neither the collision nor `methods`, so an author reads it as a mistake in their own handler. The second half is not slack -- `ConfigMethod` is the request table plus `initialize` and the built-in notifications are not in it, which is the room the hook item needs left open.",
          verification:
