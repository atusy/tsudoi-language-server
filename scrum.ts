  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        ...
        {
          criterion:
            "A HANDLER THAT THROWS LEAVES THE SESSION UNINITIALIZED, AND THE PROCESS ALIVE. The client receives an ERROR RESPONSE to `initialize`, no InitializeResult is served, stdout carries no unframed bytes, and the NEXT request is answered -32002, so a corrected `initialize` is ACCEPTED rather than refused -32600. WRITTEN FIRST AS `the phase never moved, so no rollback and no fourth phase`, AND BOTH HALVES OF THAT ARE NOW FALSE -- the handshake gained an admitted phase to refuse a concurrent second `initialize`, so there IS a fourth, and the throw path DOES roll back out of it. The criterion is what the client can observe and not the machine that delivers it, which is why it survived its own mechanism being replaced. AND ONE CLASS SITS OUTSIDE IT, MEASURED: a return value that cannot be SERIALISED fails after the transition, so the client reads -32603 and its retry -32600. Recorded rather than caught, at the site. AND THE FAILURE CONTRACT IS NOT THIS SITE'S: `exit 1, stderr, zero bytes on stdout` belongs to config load in cli.ts, before any connection exists; by initialize time stdout is LSP's, so a handler failure that exited the process would be a worse answer than the error response. THE FAILURE IS ALSO REPORTED ON STDERR, for the reason src/methods.ts already gives at reportHandlerFailure: vscode-jsonrpc consults the connection logger for NOTIFICATION handlers only, so without a line here an author's handshake handler fails where they cannot see it.",
          verification:
