...
const scrum: ScrumDashboard = {
  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        ...
        {
          criterion:
            "EVERY ITEM CARRIES `filterText`, BYTE FOR BYTE WHAT IT INSERTS. A CLIENT THAT READS THE FIELD AND DERIVES THE TYPED TEXT FROM THE EDIT RANGE -- NOT A RULE THE SPECIFICATION STATES, AND MEASURED FALSE OF SEVERAL CLIENTS BY THE REVIEW ROUND -- filters against text beginning where the FRAGMENT begins -- so with the directory part gone from the label, an item filters itself away at the keystroke that types a separator. THE STAKEHOLDER'S OWN CLIENT IS NOT WHAT THIS BUYS, which is recorded rather than assumed: ddc-source-lsp reads `insertText` and passes `filterText` over with a comment saying why, so this criterion is what keeps the change from being a regression on the clients that do read it.",
          verification:
        ...
        {
          criterion:
            "WHAT IS WRITTEN INTO THE BUFFER DOES NOT MOVE: `insertText`, `textEdit.newText` and BOTH ranges are what they were, and the widening-fragment reading of a filename holding a space is untouched. THE CONSEQUENCE THIS CLAUSE DREW FROM THAT WAS `SO NO CLIENT'S INSERTION CHANGES`, AND THE REVIEW ROUND FALSIFIED IT: a client that undoes its own edit by the LENGTH OF THE LABEL rather than by the fields held fixed here inserts differently, and eglot is one. The fields are what this criterion can bind; what a client derives from the label is not.",
          verification:
  ...
  sprint: {
    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          ...
          "AND THE CLIENT SURVEY FOUND ONE REAL REGRESSION, WHICH IS THE STAKEHOLDER'S TO RULE AND IS NOT REPAIRED HERE. In Emacs, `/` is a SYMBOL CONSTITUENT -- MEASURED in `emacs -Q` 30.2, the completion bounds over `notes/dee` are the whole fragment and not `dee` -- and eglot reverts the buffer with `(delete-region orig-pos (point))` where `orig-pos` is an INTEGER captured before the UI edited (eglot.el 30.2, the binding at :3208 and the use at :3384). A label SHORTER than the bounds therefore leaves `orig-pos` past point: `Args out of range` with nothing after the cursor, and eaten text with something after it. THIS SPRINT CREATES THAT STATE AND COULD NOT BEFORE: while the label WAS the inserted text, the typed fragment was a prefix of it by construction. The defect is eglot's -- lsp-mode does the same job with a marker and is unaffected -- but the trigger is ours, and no server-side fix keeps the short label. RULED BY NOBODY YET: it is a trade between the client the stakeholder uses, where this sprint fixed the popup and also fixed a real drop under `enableMatchLabel`, and a client nobody here runs.",
    ...
    decisions: [
      ...
      "THE CLOSING READING, RE-TAKEN AT THE END OF THE REVIEW STAGES ON THE TREE THAT CLOSES. Definition of Done PASSED at 98e99cd, all five checks exit 0: 970 pass / 0 fail over 70 files, 3036 expect() calls, 150.97s, NINETEEN registry arms HELD, ONE non-gating `eslint(require-yield)` warning at test/fixtures/throws-on-cancel.ts -- the same warning as at base. AN EARLIER SPELLING OF THIS DECISION NAMED 59babeb AS `THE TREE THAT CLOSES` AND WAS OVERTAKEN BY THREE MORE COMMITS, WHICH THE FRESH REVIEW SESSION CAUGHT: a closing reading is only a closing reading while it names HEAD, and this project has now recorded that failure twice in one sprint. THE ARM DELTA IS DECOMPOSED AND THE ASSERTION DELTA IS NOT, which is a limit of the count rather than a gap in the reading: the arms over the sprint base are the two of subtask 1, the one of subtask 2 and the three registry records, and the assertions land partly inside sweeps that run once per source and per item -- so `expect() calls` is a RUNTIME count and no source-side decomposition of it would be checkable.",
