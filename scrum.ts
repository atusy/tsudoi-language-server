  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        {
          criterion:
            "A bare `tsc --noEmit` on a checkout NOBODY HAS BUILT does not answer the framework's own published subpaths from source at exit 0 -- or the fact that it does is carried by something that stops holding the day it stops being true, rather than by prose.",
          verification:
            "RE-NARROWED AT SPRINT 58'S ACCEPTANCE TO WHAT IS LEFT, AND WHAT IS LEFT IS ONE INVOCATION. Both halves of the original criterion were met and are closed: the ABSENT and PARTIAL states are staged as written states in test/unbuilt-artifact.test.ts, read on the compiler and BOTH runtimes, and the artifact that survived a build is refused by `refuseSubpathsAnsweringFromSource` on the fifth check, naming the specifier, the file that answered and the file promised. NEITHER REACHES THE FOURTH CHECK: that command is `tsc --noEmit` at the root, it runs before nothing and after nothing, and NOTHING IN THIS REPOSITORY OWNS ITS INVOCATION.",
        },
      ],
      status: "draft",
      notes: [
        "STARTING EVIDENCE, CARRIED VERBATIM FROM SPRINT 58'S OWN READING SO NOBODY RE-MEASURES IT. Environment: bun 1.3.13, deno 2.8.3, tsc 7.0.2, base b226ecf; every cell taken BY HAND OUTSIDE `bun test`, with dist/ MOVED ASIDE after the preload would have run, because the preload builds every package before any test file loads and the state under measurement never exists during a run.",
        ...
        "FORECLOSURE ONE -- DELETING THE FRAMEWORK'S SOURCE ARMS IS REFUSED WITH MEASUREMENT, AND THE BLOCKER IS NAMED TO ITS FILE. `typeCheckProbe` in test/helpers/typecheck.ts stages the framework's manifest with src/ SYMLINKED AND NO dist/ IN EITHER STATE, so the harness this suite grades consumers with reaches the framework through the source arm; with the arms deleted the whole suite reads 875 pass / 4 fail across the same 58 files, and the four are named rather than counted -- test/package-shape.test.ts's exports equality pin, test/published-artifacts.test.ts's `the in-repo arm cannot observe what the published arm checks`, test/published-specifier.test.ts's `a config importing @atusy/tsudoi-language-server/types type-checks against the shipped package.json`, and its neighbour `a deliberate type error in the probe is reported, so the probe is really checked`, which asserts `not.toContain(\"TS2307\")` and now receives TS2307 BESIDE the TS2322 it wanted. ONLY THE FIRST IS A PIN. THE BLOCKER IS ASSERTED BESIDE THE STAGED STATES IT EXPLAINS in test/unbuilt-artifact.test.ts, and that pair reddens in BOTH directions the cost can vanish, each measured: the probe gaining a route that does not end in source is 3 pass / 1 fail, that arm alone; the arms deleted from the package is 0 pass / 4 fail, half one receiving TS2307 for the framework's own specifier.",
        "FORECLOSURE TWO -- A `paths` MAPPING OR A PROJECT REFERENCE IS REFUSED BY NAME, AND IT IS WRITTEN HERE BECAUSE WITHOUT IT THE NEXT EXECUTOR'S FIRST MOVE IS THE ONE ALREADY REFUSED. There is no mapping anywhere in this repository, `refuseMemberMappings` in scripts/workspaces.ts enforces the absence for members, and a diagnostic manufactured by either would grade A RESOLUTION NO STRANGER PERFORMS -- which is the whole reason tsudoi is a workspace member rather than the checkout root. So the remaining work is NOT a resolution fix.",
      ...
      acceptance_criteria: [
        {
          ...
          verification:
            "The readings exist and are recorded; the criterion is met by a shape whose cost at the ordinary size does not exceed the previous shape's on either runtime, or by a recorded ruling naming which runtime pays and why. STARTING EVIDENCE, TAKEN IN SPRINT 53 AFTER THE SHAPE WAS ALREADY CHOSEN, AND ITS NUMBERS RETIRED BY NAME AT SPRINT 59'S ACCEPTANCE RATHER THAN LEFT IN THE ITEM THAT CLOSES ON THEM: `45 to 127 ms at five thousand entries` and `1289 to 1977 at a hundred thousand` belong to the session sprint 59 found inflated in every figure it re-took, and they may not be cited again -- the same nominal sizes and the same two runtime versions read 6.374/9.619 and 138.507/204.036 on the tracked instrument. WHAT SURVIVES RE-TAKEN IS THE DIRECTION AND NOT THE MAGNITUDE: streaming is slower on deno at every size read, and the regression lands on the size this module's own premise calls ORDINARY. WHAT DOES NOT SURVIVE IS `bun improves at both` AS A READING OF THE SMALL SIZE -- at two hundred entries bun does not separate the shapes at all.",
        },
      ],
      status: "draft",
      notes: [
        ...
        "ACCEPTED WITH CONDITIONS AT SPRINT 59'S REVIEW, AND THIS ITEM CLOSES WHEN THE FOUR BELOW LAND -- THEY ARE THE WHOLE OF WHAT GATES THE CLOSE, and no re-decision, no new reading and no further sprint is asked for. (C1) THE LOAD-BEARING SENTENCE IS STATED WIDER THAN ITS EVIDENCE. `on deno the open allocates and DISCARDS, so an array here would be a retention it does not have today` is measured AT THE OPEN and is asserted of THE WHOLE SCAN, while the same file concedes two paragraphs earlier that what deno's LAZY read materialises once iteration has started is separated by no instrument here. Narrow it to the open AT BOTH SITES -- the ruling's `what it still buys` and the third shape's refusal, which rests on the same sentence -- because narrowing only the first leaves the shape that measured fastest on both runtimes refused on a premise stated more strongly than its evidence. (C2) A FOURTH FIRING CLAUSE, for the same reading: deno's `Dir` materialising the directory once iteration starts. It is the observation most likely to reopen this and it is absent from a list of three. If it holds, deno pays 3.261 ms for a bound it already has, which is exactly the configuration this item's criterion refuses -- so the ruling's own basis, not merely its size, is what that clause guards. (C3) THE RETIRED SPRINT-53 NUMBERS ABOVE, done in this edit. (C4) THE FLIP CLAIM IN THE SPRINT RECORD -- `the ranking on bun FLIPS WITH SIZE`, and the `all eight held` that counts it -- narrowed to what the null cell permits: bun does not separate the shapes at two hundred, prefers streaming at five thousand and the tail, and deno prefers the array at every size, so the RUNTIMES DISAGREE WITH EACH OTHER, which carries the same conclusion without a flip. IT BLOCKS THE CLOSE BECAUSE THE PREDICTION LIST IS THE ONE CLAIM IN THIS RECORD A READER CANNOT CHECK, by its own admission, and its accounting of which predictions held is the whole of its warrant. AND C1 AND C2 EDIT A SHIPPED DOCSTRING: the fourth firing clause names the RUNTIME BEHAVIOUR and not the instrument's path -- a shipped comment naming a repository file is the red this sprint already earned at test/packed-members.test.ts -- and the suite is re-run after the edit rather than reasoned about.",
        "AND WHAT THE CRITERION COULD NOT DECIDE IS RECORDED WITH THE ITEM RATHER THAN LEFT FOR THE NEXT READER TO REDISCOVER: it refuses ONE CONFIGURATION -- a runtime paying at the ordinary sizes for a benefit that appears only on the other -- and it is NOT A MAXIMISER. The shipped shape trips it only if the inversion fails, and the third shape trips it not at all. The tiebreak that actually decided this sprint was the WORKING SET plus a cancellation seam, and neither is in the criterion. IF THE FIRING CONDITION EVER FIRES, THE REOPENED ITEM NEEDS A RETENTION CRITERION WITH A MEASUREMENT BEHIND IT, not a second time criterion: this sprint's instrument reads wall-clock only, so `what is held` was argued from a signature and from INHERITED memory readings on both sides of the decision.",
      ],
    },
...

    {
      ...
      status: "draft",
      notes: [
        "FILED OUT OF SPRINT 59, WHOSE RULING SAYS THIS OF ITSELF AT ITS OWN SITE: all three clauses that would reopen it are properties of the RUNTIMES, no check reads what an `opendir` allocates, no check re-runs the instrument, and the versions every number was taken on are read off a running binary only inside that instrument. So a runtime upgrade reddens nothing and the condition is met by a person running two lines. THE RULING WAS ACCEPTED WITH THAT WRITTEN AT THE SITE -- which is the honest state and not the covered one, and disclosed and covered are different states.",
