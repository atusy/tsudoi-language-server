  ...
  product_backlog: [
    {
      ...
      notes: [
        "EVERY `BEFORE THE EDIT` IN THIS PBI MEANS COMMIT 5f7abdd, NOT THE WORKING TREE, and this one sentence is what keeps six negative controls unambiguous while the tree moves. THE SHARPEST CASE IS CRITERION 2 DIRECTION A, WHOSE EXPECTED RESULT FLIPS WITH THE BASELINE: against the CURRENT tree src/config.ts:39 calls `factory()`, so restoring a parameter to the type gives TS2554 -- but against the REVERTED tree that line passes `tsudoi`, so restoring the parameter COMPILES and no TS2554 appears. An executor who reverts first, which the ruling below authorises, would run that control, get green where red was predicted, and have NO WAY TO TELL A DEFECT FROM A BASELINE MISMATCH. Criterion 1 is exempt and needs no baseline: its ADDITION direction fails against any tree at all.",
        "SPRINT START IS BLOCKED AND PBI AUTHORING IS NOT. THE WORKING TREE WAS BEING EDITED UNDERNEATH THIS REFINEMENT, MEASURED ACROSS THREE READS IN TEMPORAL ORDER: src/types.ts:530 read `(tsudoi: Tsudoi) => Promise<TsudoiConfig>` at first read and `() => Promise<TsudoiConfig>` at a later grep; src/config.ts:39 read `(factory as TsudoiConfigFactory)(tsudoi)` at that same grep and `(factory as TsudoiConfigFactory)()` at a later targeted read. A stale cache does not produce a FORWARD PROGRESSION ACROSS TWO FILES. The PO had no shell and so CANNOT say whether this is committed or uncommitted; determining that is the first thing to do.",
        'THE TREE IS THEREFORE SUITE-RED, AND THIS IS REASONED FROM THE CODE RATHER THAN MEASURED -- no shell was available to run `bun test`. test/fixtures/snapshot-config.ts:14 still declares `(tsudoi: Tsudoi)` while src/config.ts:39 now calls `factory()`, so `tsudoi` is `undefined` INSIDE THE FIXTURE. The throw lands in the `process.on("exit")` handler rather than in the factory, so it is NOT wrapped as a ConfigError: no TSUDOI_SNAPSHOT line is written, `readMarkedLine` throws by design, and every session site through both observation fixtures fails. THE SECOND PROOF WAS ATTEMPTED AND DID NOT HOLD, recorded because a rule that only ever confirms is not being applied: `loadConfig(argv, tsudoi)` still declares a now-unused `tsudoi`, but tsconfig.json sets only `strict` and NOT `noUnusedParameters`, so tsc does not redden on that.',
        "THE PO'S RULING ON THE OUT-OF-LOOP EDIT, under the Sprint-16 improvement, which says in its own words that it applies to ANY change and not only sprint work: a partial, red, unrouted edit IS NOT AN INCREMENT and does not stand. It is EITHER committed and green -- in which case it is the Sprint-2 attachable spike and the plan says what to CHANGE about it rather than re-deriving it -- OR it is reverted and the sprint starts from a recorded state. WHAT IT MAY NOT BE is left uncommitted while criteria are authored against it, because then a negative control names a state nobody wrote down. Twelve session sites across two fixtures currently defend accepted criteria and have been broken with nothing routed back.",
