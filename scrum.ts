    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          ...
          "THE THREE GUARDS NO ARM CLAIMED WERE DECIDED ONE BY ONE AND NOT ARMED REFLEXIVELY. ARMED: the recursion refusal, because the state it forecloses ends in a chain of runs each spawning the next and there is no red at the end of that -- its substring shape is kept, both wrong directions named at the site, degenerate SAFE HALF ONLY at 14 pass / 1 fail (the throw downgraded to a returned verdict, which still spawns nothing). Commit 24ebdef. ARMED: the XML unescaping, because this suite's arm names are English sentences and the state is one apostrophe away, and what an unescaped key costs is a REFUSED that sends the author to edit a record that is right -- degenerate, the unescaping made the identity, 15 pass / 1 fail. Commit f6d1dfc. LEFT: the per-arm-file baseline key, because over one arm file `by file` and `by nothing` are one function and the probe separating them would be a record invented to exercise a Map -- what makes it fire is written at the site, and it fires LOUDLY on the first run after a second arm file lands. Commit 1334c42. AND ONE EXPRESSION DELETED RATHER THAN ARMED: a chunk split on the delimiter it came from, equal to its input on every report bun can write, taken as its own structural commit at 878 pass / 0 fail either side. Commit 85885f7.",
          ...
          "THE SAFETY RULING FROM THE DELETION EXTENDS TO A SECOND SHAPE, STATED HERE BECAUSE IT COST A DEGENERATE THIS ROUND: a perturbation whose subject is an UNBOUNDED SPAWN is reasoned about and not run, exactly as one whose subject is a recursive delete is. Deleting the recursion refusal makes the very record that arm holds spawn a run of the file that spawns a run of the file; watching it redden is lighting the fire to test the alarm, and the orphans outlive the run. The safe half of the pair is taken and the unsafe half is named -- which is the same disposition this sprint already recorded for the stage arm.",
          "WHAT THE ROUND COST THE SUITE AND WHAT IT DID NOT MOVE: the Definition of Done was taken with `bun run scripts/definition-of-done.ts` before every one of the eight commits and read five [PASSED] and warnings 1 every time; the suite went 874 -> 878 pass / 0 fail across 58 files, and no check was red at any commit.",
        ],
      },
      {
