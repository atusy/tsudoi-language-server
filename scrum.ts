...
const scrum: ScrumDashboard = {
  ...
  sprint: {
    ...
    decisions: [
      ...
      "SHIPPED IN ee53bd8..11d5963. 413 green from 405, 28 files from 27 -- EIGHT ADDED, NONE REMOVED OR WEAKENED -- each DoD command run separately and unpiped with its exit read directly. `NONE WEAKENED` IS CHECKED RATHER THAN ASSERTED, because one existing assertion WAS edited and at Review that reads the same as a loosening: test/published-artifacts.test.ts's exact-equality list gained `DiagnosticSeverity` -- ONLY THE EXPECTED VALUE MOVED, NO toEqual WAS LOOSENED, so S16 is correctly NOT engaged, and P5 measures that it still fires. The lint warnings are NAMED rather than counted, since a count holds while its members change: completion-null-only.ts:14 and throws-on-cancel.ts:44, the same two as at 2bfa2bf. FOURTH OF THE STAKEHOLDER'S FIVE METHODS, and the FIRST added through the table rather than by hand: an ENTRY, not a registration.",
      "NINE PERTURBATIONS, one at a time, reverted between, EVERY ONE THIS SPRINT NAMED. P1 (interFileDependencies true -> false) and P2 (workspaceDiagnostics false -> true) EACH redden the capability test ON BOTH RUNTIMES AND NOTHING ELSE OF 413 -- so the PO's ruling is DEFENDED VALUE BY VALUE rather than merely written down, and the two booleans are separately observable despite living in one literal.",
      "THE BY-CONSTRUCTION CANCELLATION EXPECTATION HELD, AND IT WAS MEASURED RATHER THAN REASONED THIS TIME. Sprint 31 left the residual, Sprint 32 predicted the table would dissolve it, and NO LINE WAS WRITTEN FOR THIS METHOD. P3a skips the router's REJECTION for `textDocument/diagnostic` ALONE and reddens `every method in the table is refused -32002 before initialize`; P3b skips the CANCELLATION epilogue for that method alone and reddens `every method in the table is answered -32800 when cancelled`. Both runtimes, two tests each, nothing else. THE ISOLATING PERTURBATION SPRINT 32 CALLED NO LONGER CONSTRUCTIBLE IS CONSTRUCTIBLE FOR A METHOD -- what dissolved there was isolating one COPY of a condition, and naming the method inside the shared condition is a different instrument.",
      "P4 IS THE AFFORDABILITY CLAIM'S SECOND WIRE MEASUREMENT, on a second method: replacing the fixture's `document.positionAt` with a hardcoded Position reddens THE AFFORDABILITY TEST ALONE out of 413, on both runtimes. Sprint 31 measured this for formatting and the PO recorded that they could not promise it for the remaining four; it now holds for two of them BY MEASUREMENT rather than by extrapolation.",
      "P5 REDDENS THE NAMED ASSERTION AND ALSO DOES SOMETHING UNNAMED, reported in full because half a perturbation result is the shape this project keeps catching: making DiagnosticSeverity's re-export `export type` reddens the published runtime-value assertion with a diff NAMING DiagnosticSeverity -- and ALSO stops test/diagnostic.test.ts LOADING at all, `SyntaxError: Export named 'DiagnosticSeverity' not found`, taking its 8 tests out of the run (406 ran, not 413). NOT THE SPRINT-27 OPAQUE-HARNESS DEFECT: there the only signal was a dead file, here the NAMED assertion fires anyway and prints the cause. It is the fixture doing what its doc block claims -- importing the ninth name AS A VALUE from tsudoi's own surface -- and that is a second, independent detection rather than a fault.",
      "P6 IS THE STANDING RE-RUN OF SPRINT 32'S P-D AGAINST THE FILE THIS SPRINT EDITED, and it CONFIRMS THE PO'S DELETION BY MEASUREMENT rather than by agreement: deleting the fixture's AWAITED-ONCE diagnostic handler leaves ALL 413 GREEN, while deleting its GENERATOR-DRIVEN completion handler reddens the -32800 test on both runtimes. So the note that promised `a known step rather than a surprise red` WOULD HAVE PROMISED A RED THAT CANNOT COME FOR THIS METHOD. Recorded as a residual in the same breath: nothing defends the fixture's diagnostic handler, exactly as nothing defends its formatting one.",
      "P7 ASKS WHETHER THE WIDENING COST DISCRIMINATION AND MEASURES THAT IT DID NOT -- the question a debit entry should be read with. Three variants, all refused at compile: `HoverRequest.type` in diagnostic's slot fails TS2322 on params AND result; `DocumentDiagnosticRequest.type` in formatting's slot fails the same way; and writing `generator-driven` on the diagnostic entry fails TS2322 naming the two strings, so the drive is still DERIVED and cannot be misdeclared. The error position was the only thing that moved.",
      'EXPECTED RED, DELIVERED AS A COMPILE ERROR: adding the method to MethodMap with no table entry fails TS2741 naming `"textDocument/diagnostic"` -- and the diagnostic PRINTS THE DERIVED DRIVE, `AwaitedOnceEntry<"textDocument/diagnostic">`, so the type system stated the sprint\'s own falsified premise back in the error message.',
      "NOTHING WAS EXPECTED TO REDDEN AND FAILED TO, WHICH IS ITSELF WORTH SAYING because this thread has recorded the opposite twice. Every perturbation named was run and every one flipped what it was named for. The nearest miss is P5's extra effect, which was NOT predicted and is disclosed above rather than trimmed to match the prediction.",
      ...
      "A NINTH PUBLISHED NAME ARRIVED AND THE EXACT-EQUALITY ASSERTION IS WHAT STOPPED IT ARRIVING QUIETLY: adding `DiagnosticSeverity` reddened the published runtime-value assertion BEFORE the name was written into the test, which is an assertion doing its job rather than being updated to match a change. Its test TITLE was renamed to carry BOTH names, on the Sprint-29 finding that PROSE LIVES IN TEST NAMES -- a title saying `CompletionItemKind` alone would have gone stale invisibly to any search for comment syntax. The reason for the ninth sits at src/types.ts beside the name, where the rule for a ninth says it must: a Diagnostic is constructible from a literal in EVERY member except this one, whose values are 1 through 4 and mean nothing written as numbers.",
      "THE ABSENT `identifier` IS A DECISION AND WAS NEARLY SHIPPED AS AN OMISSION, caught in self-review: DiagnosticOptions declares it optional and DocumentDiagnosticParams carries the matching one a client echoes back, so registering one creates a value tsudoi must MATCH against and TsudoiConfig has no surface to name it. THE REASONING HAD BEEN APPLIED AND RECORDED ONLY IN A TEST COMMENT, which is not a home the Lifetime Rule recognises for a decision whose violation is a CODE EDIT. Moved to the contributor, beside completion's triggerCharacters note, which is the same reasoning about the same kind of field.",
      "NO examples/ FILE, AND THE SPRINT-31 REASON IS UNCHANGED RATHER THAN RE-ARGUED: standing item 6 requires an EXECUTED example with two negative controls, and lifecycle.test.ts pins the demo config's capabilities BY EXACT EQUALITY AND BY NAME IN A TEST TITLE. THE PO'S SPRINT-31 NOTE IS NOW DUE, not overdue: they asked that when the five are done the team revisit whether the demo should demonstrate more than two. It is now FOUR METHODS AND NO EXAMPLE, which is the drift they wanted decided rather than discovered.",
