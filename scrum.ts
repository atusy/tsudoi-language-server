  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        {
          criterion:
            "A resolved FILE's `documentation` carries `source:`, `size: <n> bytes` and `lastModified:` as three labelled lines in that order; a resolved DIRECTORY carries `source:` and `lastModified:` and NO `size:` line, then its entries. The dot-separated sentence is gone from both. Falsified by a composer emitting `size:` for a directory, and by one emitting no `size:` for a file -- the second being the half the stakeholder's ruling makes cheap to lose, since a directory is now legible only by that absence.",
          verification:
            "bun test packages/tsudoi-completion-path/test/resolve.test.ts, `a directory's stat line carries no byte count, where a file's carries one` re-sited onto `size:`; THEN two perturbations, each named with the arm AND the assertion it must fail at. A size line emitted for a directory must redden that arm at its DIRECTORY value and must also redden the root arm today titled `a directory item comes back saying it is a directory, and carrying no size`, whose title this ruling falsifies and which is renamed with it. The size line withheld from a file must redden that same arm at its FILE value. A red at only one of the two does not meet this.",
        },
        {
          criterion:
            "The two arms that tell a stat-driven answer from a `kind`-driven one still do, without the word `file`/`directory` they read today: each rests on `size:` present against absent AND on the entries heading, and the arm resting on ABSENCE carries the present case in the same arm. Falsified by a handler emitting NEITHER line, which an absence-only assertion satisfies.",
          verification:
            'bun test -t "a file whose item claims to be a folder is still answered as a file" and bun test -t "a directory whose item claims to be a file still comes back with its listing", both re-sited; THEN the composer made to emit no size line for either kind, which must redden the FILE arm -- the present case. A perturbation that reddens only the directory arm leaves the degenerate this criterion names undetected.',
        },
        {
          criterion:
            "`lastModified` is truncated to the SECOND BY THE COMPOSER, so a file whose real mtime carries milliseconds renders no fractional part. Falsified by leaving the raw ISO string in place -- which EVERY FIXTURE IN THIS REPOSITORY PASSES TODAY, both stamp constants being whole seconds deliberately, so this criterion is unmet by any suite that does not stage a sub-second stamp of its own.",
          verification:
            "a new member arm staging a fractional mtime through `utimesSync`; THEN the truncation removed, which must redden THAT ARM AND NO OTHER -- the whole-second fixtures staying green is the evidence that the arm, and not the fixture, is what grades the composer.",
        },
        {
          criterion:
            "The two declared formats produce DIFFERENT bytes for BOTH kinds, a FILE included. Under the new shape the facts are the only part a file has, so the difference is at the fact join. Falsified by joining the facts with a bare newline in both formats, which makes a file's resolved block byte-identical across markdown and plaintext -- the exact vacuity the format arm was migrated out of the completion half to escape one sprint ago.",
          verification:
            'bun test -t "the markup a block is built in follows the session, not the item"; THEN the markdown fact join replaced by the plaintext one, which must redden its FILE half. A red at the DIRECTORY half alone does not meet this -- that half is discriminated by bullets it already had, and it reddens under this perturbation in both worlds.',
        },
        {
          criterion:
            "The entries heading announces truncation and nothing else: `(first <n> of <m>)` when fewer are shown than held, `(<m>)` when all are -- an EMPTY directory included, at `(0)`. The heading is emitted whenever a listing was read and never when none was, so a FILE's block still carries no listing section at all. Falsified by `(first <m> of <m>)` at the boundary, by no heading for the empty directory, and by a heading on a file.",
          verification:
            'bun test -t "a directory at or under the bound shows every entry, and an empty one says so", which already stages the boundary from the bound it just read and already carries the FILE pair; THEN the boundary spelling collapsed to one form, which must redden it AT THE EDGE AND THE EMPTY ASSERTIONS and must leave `a directory far past the bound renders twenty names and no more` GREEN -- a perturbation reddening the truncated case as well is grading the parenthetical, not the boundary.',
        },
        {
          criterion:
            "BOTH `listingSection` helpers locate the heading in BOTH format spellings, and every arm reading names asserts that the section was FOUND. Falsified by a pattern matching only the markdown heading, which returns an empty header and no names -- green, empty and silent -- for every plaintext arm: the sprint-82 anchor one level over, on the very helper that anchor was written to fix.",
          verification:
            "the pattern in packages/tsudoi-completion-path/test/resolve.test.ts narrowed to the markdown spelling, which must redden the member's PLAINTEXT listing arms by name; and the same narrowing in test/resolve-path-stat.test.ts, whose sessions declare no capabilities and so are plaintext throughout, which must redden its bounded-prefix arm. A narrowing either helper survives green is this criterion unmet, and the two move together or their own MUST NOT DISAGREE docblock is false.",
        },
        {
          criterion:
            "The prefix relation survives the reshape, for BOTH kinds and BOTH formats: the `documentation` completion sent is a strict prefix of the one resolve answers with. Falsified by inserting any part before `source:`, and by a fact join that differs between the one-part completion block and the multi-part resolved one.",
          verification:
            'bun test -t "what completion sent is a strict prefix of what resolve answers, for both kinds" and its root twin, both of which sprint 82 widened across markup AND across source and which must stay widened; THEN the registry\'s reorder weakening re-measured rather than assumed HELD, since renaming or re-siting any arm it names leaves that record DISARMED -- the direction sprint 82 measured a rename producing.',
        },
      ],
      status: "ready",
      notes: [
        "RULING ONE, THE PLAINTEXT SPELLING, AND IT IS DECIDED BY THE MARKDOWN SIDE RATHER THAN BY WHAT A TEST NEEDS. THE QUOTED BLOCK IS NOT VALID MARKDOWN FOR ITS OWN INTENT: three lines joined by a SINGLE newline are ONE PARAGRAPH in CommonMark and render as ONE RUN-ON LINE, so a markdown client would show `source: cwd size: 1062 bytes lastModified: ...`. The quoted block is therefore the PLAINTEXT spelling, and markdown needs a line break of its own whatever anyone wants for the arms. So the facts differ by format, a FILE discriminates, and the format arm keeps its file half. THE EXACT BREAK IS THE DEVELOPER'S -- a hard break, a bullet list, or bold labels -- because only the PROPERTY binds; what criterion 4 asserts is differing bytes with the red sited at the file half. REASONED FROM THE COMMONMARK RULE AND NOT MEASURED: no client was observed rendering anything here, and nothing in this repository can render markdown.",
        "WHAT A READER MUST NOT CONCLUDE FROM THAT ARM'S GREEN. It compares BYTES. It does not say a client renders the break, does not say the rendered result matches the stakeholder's block, and for a FILE it discriminates on the fact join ALONE -- so a markdown spelling that differs correctly at the join and wrongly everywhere else is green here. The DIRECTORY half's green likewise reports the bullets it already had; it says nothing about the facts. The two halves are not two readings of one claim and neither may be deleted as redundant.",
        "RULING TWO, THE HEADING AT THE BOUNDARY. `(first <n> of <m>)` when truncated, `(<m>)` when not, `(0)` for the empty directory. `first 67 of 67` answers the one question the parenthetical exists to answer with a word that means the opposite. TWO SPELLINGS SWITCHING AT THE BOUNDARY TODAY'S SENTENCE ALREADY SWITCHES AT, deliberately: no new boundary is introduced, so the edge arm that stages the boundary from the bound it just read keeps grading exactly what it grades now. THE EMPTY DIRECTORY STILL SAYS SO, which is not decoration -- with names alone, `this directory holds nothing` and `nothing was listed` are the same bytes, and the file pair beside it is what makes `(0)` mean something.",
        "THE SWEEP RULE THIS ITEM IS WRITTEN UNDER, AND IT IS SPRINT 82'S LARGEST FINDING APPLIED TO THE ITEM WRITTEN AFTER IT. The composer can read the FORMAT and the KIND, and this change moves both at once. A relation asserted over one value of either is a green about that value: a composer reshaping FOR MARKDOWN ONLY, or FOR DIRECTORIES ONLY, must not pass. That is why criteria 1, 2, 4 and 5 each name WHICH value the red must land at rather than that something reddens -- PBI-82's criterion 6 shipped naming a perturbation that reddened at a membership assertion predating the sprint and had to be amended at closing, and `something went red` is what let it through.",
        "WHAT THIS MUST NOT BREAK, CARRIED FROM SPRINT 82 AND EACH NOW OWNED BY A CRITERION RATHER THAN BY A NOTE: the prefix relation (criterion 7), the directory-carries-no-byte-count ruling (criterion 1), and both `listingSection` helpers (criterion 6). The third was the one at risk of being left to a docblock, and a helper returning empty is green, empty and silent.",
        "THE STAKEHOLDER'S TWO RULINGS ARE RECORDED WITH THEIR COST RATHER THAN AS PREFERENCES. NO `kind:` LINE: the objection was put -- two arms landed last sprint read the word `file`/`directory` -- and was OVERRULED, so re-siting them onto `size:` present-against-absent plus the heading is this item's work and criterion 2 is the whole of what stops absence-only assertions replacing them. `lastModified` TRUNCATES IN THE COMPOSER: criterion 3 exists because truncating whole seconds is a NO-OP, so every fixture here passes an untruncated composer and the ruling would ship untested without an arm staging a fractional stamp.",
        "WHAT IS DELIBERATELY NOT A CRITERION HERE. Sprint 83 also owes the two `sprint`-timed improvements from sprint 82's retrospective -- the narrowed weakenings and `redAt` on `PerturbationRecord` -- which are mechanism work on test/perturbations.test.ts and test/helpers/perturbation.ts. They do not compete with these criteria and are NOT duplicated by any of them; criterion 7's re-measurement requirement is about a record going DISARMED under a rename, which is the instrument as it stands today. This dashboard's header records eighty-one improvements accumulating across thirty-eight retrospectives with not one closed; carrying these past one boundary is instances one and two of that.",
        "DISSENT-WORTHY, AND THE PRODUCT OWNER RULED RATHER THAN ASKED. ONE: markdown gets its own fact spelling instead of the format claim being scoped to directories with the file half deleted. A reviewer could hold that the stakeholder quoted a block with single newlines and that honouring it literally in both formats is the instruction -- it is read as the PLAINTEXT spelling because in markdown it does not render as the thing they drew, and a block that renders as one run-on line fails the item's own benefit. TWO: `(<m>)` rather than `(all <m>)` or `(<m> of <m>)`; the shortest form that answers the question, and cheap to change since criterion 5 pins the boundary behaviour and not the word. THREE: the labels stay in the stakeholder's exact spelling, `lastModified` in camelCase beside lowercase `source` and `size`, because it is theirs and consistency is not worth a silent edit to a thing they wrote out.",
      ],
    },
    {
      id: "PBI-84",
  ...
  sprint: {
    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          "THE TRUNCATION IS A FLOOR OF THE VALUE AND NOT A CUT OF THE STRING, AND THIS IS THE ONE PLACE THE ITEM CONTRADICTED ITSELF -- RESOLVED BY THE VERIFICATION, WHICH IS THE HALF A PERTURBATION CAN GRADE. Criterion 3's PROSE says a fractional mtime `renders no fractional part`, which reads as cutting `.mmm` off the ISO string. Its VERIFICATION says removing the truncation must redden the new arm AND NO OTHER, and the sprint decision says truncating a whole second is A NO-OP. Those two are only true of a VALUE floor: MEASURED, `2001-02-03T04:05:06.000Z` floored to the second renders byte-identically, where cutting the string yields `...06Z` and would move EVERY expected block in both suites -- so under the prose reading the required perturbation reddens most of two suites and the criterion cannot be met as written. Flooring is what shipped: a fractional stamp renders `...06.000Z`, so the milliseconds the disk kept are gone and a constant `.000` remains. THE DEPARTURE IS FROM THE PHRASE `no fractional part` AND IS RECORDED HERE RATHER THAN ABSORBED.",
    ...
    decisions: [
      ...
      "TRUNCATING WHOLE SECONDS IS A NO-OP, WHICH NOBODY HAD STATED AND WHICH IS WHY THE TRUNCATION NEEDS AN ARM OF ITS OWN. Both stamp constants in this repository are whole seconds BY DESIGN -- filesystems disagree about sub-second precision -- so an untruncated composer passes every fixture that exists today and the stakeholder's ruling would ship untested. The new arm stages a FRACTIONAL mtime and asserts its own premise first, because a filesystem that truncates sub-second precision itself would make the arm measure nothing and must say so rather than pass.",
