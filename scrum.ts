        ...
        commits: [
          {
            hash: "2d6f1de",
            message:
              "feat(completion): withdraw the tuple -- one slot, one meaning, and the token decides",
            phase: "green",
          },
        ],
        notes: [
          "P1 REDDENED THE RIGHT ASSERTION AND TWENTY-TWO TESTS, NOT THE FOUR PREDICTED, AND THE DEVIATION IS THE MOST USEFUL RESULT OF THE SPRINT. EXPECTED: the token-present-one-batch arm and the throwing-handler test, four tests over two runtimes. OBSERVED: 433 pass / 22 fail. THE TARGET FIRED EXACTLY AS REQUIRED -- `with a token, a stream that yields once ...` reddens AT `expect(afterYield.progressCount).toBe(1)`, `Expected: 1, Received: 0`, NAMING THE PROGRESS COUNT, which is what the arm's assertion order was built for. THE EIGHTEEN UNPREDICTED ONES SHARE ONE MECHANISM, verified rather than assumed by reading a failure: `timed out after 2000ms waiting for 1 $/progress; saw 0`. Holding the first batch to see whether a second arrives means every PARKING fixture's first batch waits on a pull that is behind a gate the test has not opened yet -- so the gate, cleanup, cancellation and workspace tests deadlock. THAT IS THE COST THE GOAL CITES FOR REFUSING THE LOOK-AHEAD, ARRIVING AS A MEASUREMENT: `the FIRST batch waits on the SECOND pull, a delay that lands exactly when the first chunk is slow`. The suite demonstrates the design's stated reason, which the prediction did not anticipate because it reasoned about WHICH ARMS the rule changes and not about WHEN A HELD BATCH IS RELEASED.",
          "P2 AND P3 WENT AS PREDICTED, each named by the assertion it flips. P2, aggregate under a token: 423 pass / 32 fail, EVERY token-present arm red -- `each batch reaches the client as its own $/progress ...`, the one-batch arm, the appends-exactly-once guard, both protocol token tests and the path example's batching -- and NO token-absent arm. P3, stream under an invented token when none was sent: 428 pass / 27 fail, BOTH token-absent arms red -- `without a partialResultToken every batch arrives as one response` and `... one batch is the whole response` -- and no token-present arm. The two together are what make the rule two-sided: presence alone would be satisfied by a drive that always streams.",
          "THE STANDING ITEM 6 CONTROL WAS RE-RUN PER SPRINT 14 AND THE FIRST PROBE WAS DEGENERATE, recorded because its green read exactly like success. Perturbing the `if (!document)` arm of examples/tsudoi.config.ts to `yield []` left test/completion.test.ts ENTIRELY GREEN -- and that is not the control gone quiet: THAT REQUEST NEVER REACHES THAT ARM, because the document IS in the store and the `null` comes from there being no path fragment at character 0. Sprint 42's retrospective asks precisely this before a green is read. RE-PROBED AT THE SITE THE `null` ACTUALLY COMES FROM, `yield []` at pathCompletion's own exit in examples/completion-path.ts: reddens AT `expect(nothing).toBeNull()` with `Received: []`, two tests, one per runtime, and nowhere else. FOURTH DEGENERATE PROBE IN THIS THREAD and the fourth to produce a clean green.",
          "THE `expect(` DIFF LANDED ON THE PREDICTION, 712 -> 708 source, 1302 -> 1294 runtime, 459 -> 455 tests, with test/completion.test.ts 40 -> 36 and EVERY OTHER FILE AT ZERO exactly as forecast. ONE DEVIATION, found and removed: a first draft read 709 because A COMMENT QUOTING THE DEAD ASSERTION IS COUNTED BY THE INSTRUMENT -- `grep 'expect('` cannot tell an assertion from prose about one. The runtime count and the test count were exact throughout, which is what localised it. FILED AS AN INSTRUMENT FINDING rather than a slip: this project measures `none weakened` with that grep, so any sprint whose commentary quotes an assertion inflates its own diff, and the site now says so.",
        ],
      },
      {
        ...
        commits: [
          {
            hash: "108daa8",
            message:
              "docs(completion): keep the loss on the record, and name what would restore it",
            phase: "green",
          },
        ],
        notes: [
          "THE GAP'S OWN CONTROL RAN IN BOTH DIRECTIONS AND THE VERDICTS ARE UNCHANGED. BEFORE, at 7bd435f, `grep -rn 'COMPLETENESS RULING: NOT COMPLETE'` returned THREE: examples/tsudoi.config.ts:21, examples/completion-path.ts:454 and test/fixtures/completion-list.ts:24. AFTER: TWO, the same two examples, both still NOT COMPLETE. THE THIRD IS ACCOUNTED FOR RATHER THAN LOST -- completion-list.ts is the fixture whose whole purpose was to say `not complete` ON THE WIRE, which no shape can now do, so it was deleted with its test as TARGET DELIBERATELY REMOVED. A shrinking verdict count is exactly what a quiet relabelling would look like, which is why it is enumerated here by file rather than counted.",
          "CRITERION 4'S CONTROL DISCRIMINATES PRESENT FROM ABSENT, which is the only thing it can fail at. The anchor `THE FUTURE PATH` was chosen BECAUSE the before-grep against the baseline returns NOTHING across src/, test/, examples/ and README.md -- exit 1, zero lines -- and the obvious anchor `CompletionItem[] | CompletionList` was REJECTED for the opposite reason: it already had four hits at baseline, so a grep on it could not have told the note's arrival from the prose that was already there. AFTER: two hits, src/types.ts:106 and examples/completion-path.ts:478.",
          "CRITERION 1'S SURFACE COMPARISON RAN WITH ITS NEGATIVE CONTROL, and the control was taken through the SAME comparison rather than by inspection. The published names of dist/types.d.ts go from TEN to EIGHT, losing EXACTLY `CompletionResponse` and `EmptyCompletionResponse` and gaining none. NEGATIVE CONTROL: a throwaway `export type ThrowawayControlProbe` added to src/types.ts, REBUILT, and the comparison REPORTS it as an addition; removed, rebuilt, and the surface is byte-identical to the eight again. THE REBUILD IS LOAD-BEARING AND IS WHY THE CONTROL IS NOT DEGENERATE: dist/ is a built artifact, so a comparison run without `tsc -p tsconfig.build.json` would have compared the probe against itself.",
          "THE SPRINT-42 TRAP FIRED ON THIS EXECUTOR, CAUGHT BY THE PRACTICE THE RETROSPECTIVE FILED. After the fixture and example rewrites `tsc --noEmit` reported TS2322 at examples/tsudoi.config.ts saying the handler was not assignable to a PROMISE OF A TUPLE -- a type this tree no longer contains. The examples resolve `@atusy/tsudoi/types` to dist/types.d.ts, which still held the withdrawn shape. `tsc -p tsconfig.build.json` then exit 0, and the phantom error was gone. THIRD MEASUREMENT OF THIS CLASS AND THE FIRST WHERE THE STALE ARTIFACT REPORTED A FAILURE THAT DID NOT EXIST -- Sprint 42's two were a false GREEN and a false red on a clean src/. The instrument lies in both directions.",
        ],
      },
    ],
    impediments: [],
