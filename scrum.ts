  ...
  product_backlog: [
    {
      ...
      notes: [
        "FILED AT SPRINT 43'S REVIEW. HOMELESS SINCE SPRINT 42, WHICH SAID SO TWICE IN ITS OWN RECORD AND IN ITS RETROSPECTIVE, and a finding whose only home is an active improvement labelled `until it has a home` is a finding waiting to be forgotten. THE PRACTICE IT REPLACED -- `after any change to the published types, run tsc -p tsconfig.build.json BEFORE believing tsc --noEmit` -- IS RETIRED BY CRITERION 4 AND NO LONGER DESCRIBES ANYTHING, because the mapping means `tsc --noEmit` never reads the built artifact. CORRECTED BY THE PRODUCT OWNER AT SPRINT 44'S REVIEW: this note said the practice was unchanged and not superseded, which was true when it was filed and went false the moment the increment landed. It is corrected rather than left to evaporate with the PBI, because a false sentence that reads as protection is the Sprint-22 finding and does not stop being one because its carrier is short-lived.",
