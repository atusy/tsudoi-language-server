  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        ...
        {
          criterion:
            "Each DIRECTION of factory-arity agreement is MEASURED and then labelled at `TsudoiConfigFactory` per the Sprint-11 classification. The two directions are NOT the same question: (A) the type's arity against the arity tsudoi actually calls with, and (B) a config author's own default export against the type. DIRECTION B'S ANSWER IS ROUTE-DEPENDENT BY DESIGN, at the stakeholder's direction: the DOCUMENTED route -- the README quickstart and examples/tsudoi.config.ts -- carries `satisfies TsudoiConfigFactory`, so an author following it IS caught; an author who omits it is NOT, AND CANNOT BE, because their file is theirs and this project never sees it. THE LABEL IS `DEFENDED ON THE DOCUMENTED ROUTE, UNDEFENDED ON THE REST`, AND IT IS DELIBERATELY NOT SPRINT 40'S `IMPOSSIBLE ON EVERY DOCUMENTED ROUTE, DETECTED ON THE REST`: that ruling's bypass route still carried A ROT DETECTOR, and this one carries NOTHING. Writing the stronger phrase here would overstate the guarantee in the permanent record. AND THE README MUST SAY WHAT `satisfies` BUYS rather than only showing it -- one clause, that it makes tsudoi tell you when the config shape changes -- because an unexplained token in the quickstart is the exact defect this PBI exists to remove, and replacing `_tsudoi` with a second inexplicable token would be a lateral move.",
          verification:
            "DIRECTION A: restore a parameter to `TsudoiConfigFactory` ALONE and run `tsc --noEmit` unpiped. Expected TS2554 at src/config.ts:39, which passes zero arguments -- a RED here is the PASS. DIRECTION B, THREE PARTS, AND THE FIRST TWO HAVE OPPOSITE EXPECTED RESULTS. B1, THE OPT-IN HALF, MEASURED BY THE SCRUM MASTER UNPIPED BEFORE THIS TEXT WAS WRITTEN: a config written the OLD way, `(_tsudoi: Tsudoi) => Promise<TsudoiConfig>`, with `satisfies TsudoiConfigFactory` against the zero-argument type gives EXIT 1 and TS1360 `Target signature provides too few arguments. Expected 1 or more, but got 0.` ITS POSITIVE PAIRING, TAKEN IN THE SAME SESSION so the probe is not one-directional: a zero-argument config with the same `satisfies` gives EXIT 0. B2, THE OPT-OUT HALF, AND ITS GREEN IS THE RESULT RATHER THAN A NON-FINDING: give ONE fixture's default export a `(tsudoi: Tsudoi)` parameter WITHOUT `satisfies`, and all four DoD checks must be GREEN -- src/config.ts:39 reaches the type only through a CAST from `unknown`, so nothing catches it. ITS NEGATIVE CONTROL, without which that green reports nothing: perturb the SAME fixture to a shape that IS caught -- export the config OBJECT instead of a function -- and confirm it is refused with the named ConfigError, proving the loader can see the file at all. B3, THE DOCUMENTED ROUTE IS PROVEN AND NOT MERELY TAUGHT, which is what makes B1 a permanent pairing under Sprint 6 rather than a probe taken once: examples/tsudoi.config.ts carries `satisfies` and is compiled by the DoD's `tsc --noEmit` on EVERY run, per standing item 6. ITS CONTROL, because a decorative `satisfies` would compile forever and defend nothing: perturb THAT EXAMPLE to the old one-parameter shape while KEEPING the `satisfies` clause, and tsc must redden with TS1360. A control that can never be the first thing to fail is not a control.",
        },
        {
          criterion:
