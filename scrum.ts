      ...
      acceptance_criteria: [
        {
          ...
          verification:
            "Drive a real client -- nvim is the one this repository has measured against before, at the workspaceFolders normalisation ruling -- and observe whether it RE-QUERIES on the next keystroke. NEGATIVE CONTROL, AND IT IS THE LOAD-BEARING HALF: a response carrying BOTH items AND `isIncomplete: true` must be observed to trigger a re-query FIRST. Without it, `the client did not re-query` cannot distinguish IGNORES `isIncomplete` ENTIRELY from IGNORES IT ONLY WHEN ITEMS ARE EMPTY -- two outcomes, one observation, which is the Sprint-9 widening and records nothing. IF THE CONTROL DOES NOT FIRE, the client cannot see `isIncomplete` at all and this whole PBI is measuring the wrong instrument; say so and stop. THE PRECEDENT THAT MAKES THIS NON-OPTIONAL is this project's own: a `$/progress` addressed to a token no client could correlate was SILENT MISDELIVERY, and shipping `isIncomplete` on an empty-items response that no client reads would be the same class -- a feature that appears to work and does nothing.",
        },
        {
          criterion:
