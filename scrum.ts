  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        ...
        },
        {
          ...
          verification:
            "a new member arm staging a fractional mtime through `utimesSync`, asserting that NOTHING FRACTIONAL is rendered at all -- a claim no whole-second fixture can make, since a composer that merely FLOORED THE VALUE satisfies `the file's own milliseconds are gone` and still renders `.000`; THEN the truncation removed, which must redden THAT ARM at its own assertion. `AND NO OTHER` STOOD HERE AND IS STRUCK: it rested on the sprint decision that truncating a whole second is a no-op, which is TRUE OF THE VALUE and FALSE OF THE STRING, and the two were conflated when this was written. Cutting the string moves every rendered byte, so the perturbation reddens every whole-value arm beside this one; what makes it grade THIS criterion is `redAt` naming the fractional arm's own assertion, which is the instrument this sprint built first and the reason it was built first.",
        },
        {
          criterion:
