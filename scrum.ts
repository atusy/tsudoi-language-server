    ...
    subtasks: [
      ...
      },
      {
        ...
        notes: [
          ...
          "THE RATE, NOT THE REPAIRS. This sprint's prose kept making claims about its own text that the text did not support -- in the sprint whose subject is that class. CAUGHT BEFORE LANDING: the arm's first draft said DELETE, NARROW and SUPERSEDE all fire on a fact having CHANGED, which DELETE's own text refutes. LANDED IN THE ARM AS SHIPPED: `Everything above in this file is what a mutable present-tense home costs`, over a corpus whose entries include prose false on arrival -- found by a reviewer, F2. LANDED IN A REPAIR: `the five in test/published-artifacts.test.ts`, a count copied rather than taken, six on measurement -- found by me re-reading my own repair. LANDED IN A REPAIR: `an ORDERING over homes this section already carries`, wrong about the entries it inherited AND about the entries it added -- found by a reviewer, F11. LANDED IN F11'S OWN REPAIR: `restate`, true of THE COMMIT's measurement trigger and false of its foreclosed-alternative one -- found here, F12. MOST OF WHAT LANDED WAS INTRODUCED BY THE REPAIR PASS ITSELF, and each of those repairs had been reviewed as sound when it was made. NO TOTAL IS WRITTEN because the round is still open, and a total taken mid-round is the count this sprint has twice deleted.",
          "F12, FILED AND NOT RESOLVED, THROUGH THE ARM SUBTASK 2 SHIPPED. `.claude/skills/writing-a-comment/SKILL.md` now assigns a FORECLOSED ALTERNATIVE to two homes: :249-250 gives it a line at the site where someone would otherwise reintroduce it, and the sprint-81 bullet gives it THE COMMIT. Both are live and the file now says so at the list rather than letting a reader meet either alone. NOT DECIDED HERE: the bullet's trigger is PBI-81 criterion 2 verbatim and is the product owner's, and :249-250 is sprint-65 text with its own reason -- a developer picking one would be overruling one of them from inside a repair round. THE FILE'S OWN CLASS, quoted because it is the reason this is written down at all: a comment contradicting another comment in the same repository is a detectable condition nobody detects.",
