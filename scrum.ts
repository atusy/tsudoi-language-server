...
const scrum: ScrumDashboard = {
  ...
  product_backlog: [
    {
      ...
      notes: [
        ...
        "`THE COST IS LINEAR` WAS WRITTEN HERE AND IS FALSE, CAUGHT BY THE SECOND INDEPENDENT REVIEWER AND RETIRED RATHER THAN EDITED QUIETLY: reading every name and SORTING it is not linear, and the sentence was the justification for accepting an unbounded directory. MEASURED AT A HUNDRED THOUSAND ENTRIES, which the original reading did not cover: the array shape costs 888 ms on bun and 1289 ms on deno, OF WHICH THE SORT ALONE IS 515 and 386 -- so what was accepted was accepted on a number that was never taken. THE ANSWER WAS TO BOUND THE WORKING SET rather than to keep the acceptance: the entries are streamed and only the best twenty are retained, which keeps the whole directory read (and so the exact total) while nothing of the directory's size is held or sorted. The tail is still linear in READS and no longer in memory or in comparisons.",
  ...
  sprint: {
    ...
    decisions: [
      ...
      "THE ACCEPTANCE OF AN UNBOUNDED DIRECTORY RESTED ON A SENTENCE THAT WAS FALSE, AND THE SECOND INDEPENDENT REVIEWER IS WHAT CAUGHT IT: `the cost is linear` -- while the shape read every name into an array and SORTED it. MEASURED at a hundred thousand entries, the size the original reading never took: 888 ms on bun and 1289 ms on deno, the sort alone 515 and 386. THE WORKING SET IS NOW BOUNDED INSTEAD OF THE ACCEPTANCE BEING KEPT: entries are streamed and only the best twenty retained, so the whole directory is still read -- which is what lets the answer state an exact total -- and nothing of the directory's size is held or compared. WHAT THIS COSTS AND IT IS WRITTEN DOWN: streaming is SLOWER ON DENO AT MIDDLING SIZES (127 ms against 45 ms at five thousand), which is the same order as the number the original ruling was made on.",
