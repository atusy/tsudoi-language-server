  ...
  product_backlog: [
    {
      ...
      status: "done",
      notes: [
        "RETIRED AS A RECORDED DECISION WITH ITS RESIDUE NAMED, ON THE BRANCH THE MEASUREMENT SELECTED AND NOT THE ONE THE SPRINT PREFERRED. The criterion is a disjunction -- close the route, or name the state the day it arises -- and the CLOSURE half is exhausted across all three shapes, each on its own ground: a precondition on the framework's ABSENT artifact selects the grade that hides a disagreement; a `prepack` that builds the framework first puts a cross-package build into a manifest that travels to a registry, AND SO DOES A DETECTOR RATHER THAN A BUILDER, which is the shape that would otherwise be proposed as the cheap version; and freshening from the WORKSPACE side adds a safe route BESIDE the unsafe one, which bare `bun pm pack` in the member bypasses. THE NAMING HALF WAS TAKEN AT ITS ONLY AFFORDABLE WIDTH: the state is written where a `prepack` reason lives -- each handler's own test/package-shape.test.ts -- and staged as a re-runnable arm at test/stale-framework-artifact.test.ts.",
        "THE RESIDUE, NAMED SO THE RETIREMENT IS NOT READ AS A DELIVERY. NOTHING FIRES ON THE STATE THE DAY IT ARISES, and the two routes that would have to are ruled out BY THEIR OWN ORDER rather than by a search: an arm under `bun test` runs after a preload that has just rebuilt every package, and the fifth Definition-of-Done check calls `prepareWorkspace` before it reads. Both stand after a build; this state is before one. What a maintainer meets is a paragraph, and paragraphs are what this project has measured being missed.",
        "AND WHAT THE MEASUREMENT COVERS IS NARROWER THAN THE ITEM'S SENTENCE, which is the other half of why this is a retirement: one perturbation shape in one direction, on ONE OBSERVATION TAKEN TWICE -- both handlers declare tsudoi the same way and consume the same handler type, so the pair is not breadth. `A stale artifact hides a narrowed return type` is what may be written; `hides a changed shape` is not.",
        "WHAT IS ALREADY MEASURED, so the first subtask starts from it rather than from zero: with an exported type RENAMED in the framework's src/ alone, a handler built against the stale dist/ EXITS 0 while the same build against src/ EXITS 2 with TS2305. So the stale artifact is the grade that HIDES a disagreement. THE STRUCTURAL CELL IS NO LONGER UNTAKEN, AT ONE SHAPE AND IN ONE DIRECTION, SUPERSEDED IN PLACE RATHER THAN WIDENED: a NARROWED RETURN -- the handler type's return intersected with a required marker, its NAME kept -- measured at base d2d6519 on bun 1.3.13 / tsc 7.0.2, where a handler EXITS 0 against the framework artifact that predates the edit and EXITS 2 with TS2322 against one rebuilt after it, and `bun pm pack` in the stale cell EXITS 0 and produces its tarball. NOT `structural changes are hidden`: a widened return, a changed parameter type, a renamed property inside an object type and a changed generic constraint are UNMEASURED, and the EMITTED content, any consumer-side compile of it and the resolution trace are UNREAD.",
    ...
    subtasks: [
      ...
      },
      {
        ...
        commits: [
          {
            hash: "9aad8c6",
            message: "docs(handlers): PBI-76 ends as a named residue, where a prepack reason lives",
            phase: "green",
          },
        ],
        notes: [
