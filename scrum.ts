...
const scrum: ScrumDashboard = {
  ...
  product_backlog: [
    {
      ...
      notes: [
        ...
        "`THE COST IS LINEAR` WAS WRITTEN HERE AND IS FALSE, CAUGHT BY THE SECOND INDEPENDENT REVIEWER AND RETIRED RATHER THAN EDITED QUIETLY: reading every name and SORTING it is not linear, and the sentence was the justification for accepting an unbounded directory. MEASURED AT A HUNDRED THOUSAND ENTRIES, which the original reading did not cover: the array shape costs 888 ms on bun and 1289 ms on deno, OF WHICH THE SORT ALONE IS 515 and 386 -- so what was accepted was accepted on a number that was never taken. THE ANSWER WAS TO BOUND WHAT THIS PACKAGE RETAINS rather than to keep the acceptance: the entries are streamed and only the best twenty are kept, so the whole directory is still read -- which is what lets the answer state an exact total -- and no array of the directory's size is built or sorted HERE. AND MY OWN REPLACEMENT SENTENCE WAS FALSE IN ITS TURN, caught by the same reviewer on the next round: `nothing of the directory's size is held or compared` is wrong twice. COMPARISONS ARE STILL LINEAR -- every entry is compared against the worst one kept. AND THE RUNTIME HOLDS THE DIRECTORY WHATEVER THIS CODE DOES: measured at a hundred thousand entries, bun's handle is a facade over a full read (30 MB at the open) and deno's `opendir` calls a SYNCHRONOUS full read to fail early (57 to 119 MB at the open), so neither streams from the kernel. What is bounded is this package's retention and its sort; the runtime's allocation is not, and saying otherwise was the third false complexity claim on one paragraph.",
        ...
        "FD RELEASE ON AN EARLY EXIT IS NOW MEASURED, AND THE READING REFUSED A FIX RATHER THAN GUIDING ONE. ON DENO A DIRECTORY THAT HAS BEEN READ FROM AND NOT DRAINED NEVER RETURNS ITS DESCRIPTOR: five hundred listings abandoned after one entry take the process from 21 open descriptors to 521, IDENTICALLY for a return, for a break, and for a break followed by an awaited close. Opening and closing without reading leaks nothing; a full drain leaks nothing; THE PARTIAL READ ALONE DOES. Bun releases in every shape. That is why the second cancellation checkpoint the reviewer asked for -- between the listing and the sort -- IS REFUSED: one descriptor per cancelled highlight ends the session at the ulimit, which is worse than the drain it would save. Release by EXHAUSTION is what the streaming shape now relies on, measured over two thousand resolves on both runtimes with the count unmoved.",
  ...
  sprint: {
    ...
    decisions: [
      ...
      "THE ACCEPTANCE OF AN UNBOUNDED DIRECTORY RESTED ON A SENTENCE THAT WAS FALSE, AND THE SECOND INDEPENDENT REVIEWER IS WHAT CAUGHT IT: `the cost is linear` -- while the shape read every name into an array and SORTED it. MEASURED at a hundred thousand entries, the size the original reading never took: 888 ms on bun and 1289 ms on deno, the sort alone 515 and 386. WHAT THIS PACKAGE RETAINS IS NOW BOUNDED INSTEAD OF THE ACCEPTANCE BEING KEPT: entries are streamed and only the best twenty kept, so the whole directory is still read -- which is what lets the answer state an exact total -- and no array of its size is built or sorted here. THE SENTENCE THAT REPLACED THE FIRST FALSE ONE WAS FALSE TOO, and this record is the third place on this subject to need repair: comparisons remain LINEAR, and BOTH RUNTIMES MATERIALISE THE WHOLE DIRECTORY BEHIND THE HANDLE regardless -- measured, and by different routes, one a facade over a full read and the other a synchronous full read taken so the open can fail early. A THIRD PARTY MEASURED IT AND WAS ALSO WRONG: the reviewer attributed the residue to a thirty-two entry buffer, and reading the two runtimes' own directory objects says no such buffer is used. WHAT THIS COSTS AND IT IS WRITTEN DOWN: streaming is SLOWER ON DENO AT MIDDLING SIZES (127 ms against 45 ms at five thousand), the same order as the number the original ruling was made on.",
