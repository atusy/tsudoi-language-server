  ...
  product_backlog: [
    {
      ...
      acceptance_criteria: [
        ...
        },
        {
          ...
          verification:
            "THE FALLBACK IS ALL-OR-NOTHING: on the FIRST change notification the synthesised list is DISCARDED IN ITS ENTIRETY, and the delta applies to what the client sends. Assert the synthesised folder is gone and the client's own list stands alone. STATED AS A VALUE, NOT A MECHANISM, and the restatement is what dissolves the tension with the no-distinguishable-entries ruling: `discard the synthesised list` needs NO identity tracking, where `drop the synthesised entry` implied knowing which entry it was. It also forecloses an ordering hazard the first wording invited -- if the client's first change ADDS a folder at the URI we synthesised, a drop-by-URI would discard the CLIENT'S entry. NEGATIVE CONTROL: keeping the fallback leaves the list permanently part ours and part theirs",
        },
      ],
      status: "ready",
      notes: [
        ...
        "`name` IS SYNTHESISED AS THE FULL PATH, decided on consistency from measurement rather than from the spec, which says nothing: nvim sets `name` to the FULL PATH and not a basename, so `fileURLToPath(rootUri)` -- and rootPath verbatim, already a path -- makes a fallback entry INDISTINGUISHABLE IN SHAPE from that client's real entries. A config author should not be able to spot the fallback by how it looks.",
