  ...
  describe(runtime.name, () => {
    ...
    test("the example config is driven end to end and answers from the document's own directory", async () => {
      ...
      try {
        ...
        // A gated query can gain candidates as the user types.
        ...
        // An unavailable document still gives no answer.
        const unavailable = await session.request<null>("textDocument/completion", {
          textDocument: { uri: `${documentUri}.unopened` },
          position: { line: 0, character: 0 },
        });
