...
record({ runtime: runtimeName, args });
...
function serveLsp(): void {
  ...
  process.stdin.on("data", (chunk: Buffer) => {
    ...
    while (true) {
      ...
      record({ runtime: runtimeName, lsp: message.method });
      ...
      if (message.method === "initialize") {
        ...
        result = [{ label: "registry", detail: "around" }];
...
if (runtimeName === "bun" && args[0] === "add") {
  ...
  if (!config.includes("completeAround") || !config.includes("TsudoiConfigFactory"))
    process.exit(5);
} else if (
