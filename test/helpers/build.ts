 ...
 *
 * WHO NEEDS THE FRAMEWORK'S dist/: EVERYTHING, WHERE IT USED TO BE `THE ARMS
 * THAT SPAWN DENO`. MEASURED on this tree with dist/
 ...
 * rewrite of the emitted declarations added HERE leaves the full suite green and
 * the checkout's handler dist/ carrying the unrewritten text, while
 * the same rewrite in a handler's `prepack` reddens arms. What it costs
 ...
 * THIS PACKAGE'S. Root `tsc --noEmit`, tsc 7.0.2, base 6d1c85d, each cell with
 * dist/ MOVED ASIDE. Everything built: exit 0, silent. Nothing built: exit 1
 ...
 *
 * AND THAT SILENT CELL IS NOT HAND-MADE: two documented commands in an order
 * nothing forbids
 ...
 *
 * The throw on a non-zero exit is deliberate: a suite that ran on the previous
 ...
 *
 * So a freshly wrong dist/ that still parses is what the root
