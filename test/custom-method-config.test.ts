 ...
 * the handler never run -- and every refusal `customMethods` gains is green under
 * exactly that fault, refusing what it must and carrying nothing.
 *
 * BOTH KEYS IN ONE READING, because the drop is directional: a `customMethods`
...
test("a declared custom method survives config load, and reading it does not drop methods", async () => {
  ...
  expect(typeof config.customMethods?.["textDocument/didFocus"]).toBe("function");
...
test("a config declaring no custom method carries none", async () => {
  ...
  expect(config.customMethods).toBeUndefined();
});
