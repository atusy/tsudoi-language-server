...
async function completedItems(session: LspSession, root: string): Promise<CompletionItem[]> {
  ...
  // No partialResultToken: the yields are aggregated into the response, which
  // is the shape a client without partial-result support receives.
  const items = await session.request<CompletionItem[] | null>("textDocument/completion", {
    textDocument: { uri },
