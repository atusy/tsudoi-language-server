 ...
 * current working directory: a tree under tests/ would put a second [test]
...
const helperDeadlines = readdirSync(join(repoRoot, "tests", "helpers"))
  .filter((name) => name.endsWith(".ts") && name !== "deadline.ts")
  .flatMap((name) =>
    numeralsIn(readFileSync(join(repoRoot, "tests", "helpers", name), "utf8")).map((ms) => ({
      file: name,
      ms,
    })),
  );
 ...
 * tests/helpers/fake-editor.ts, because its timer is not a deadline a test
 ...
 * BECAUSE BUN DISCOVERS RECURSIVELY: a `.test.ts` under tests/fixtures/, under
...
/** Integration and E2E suites use the shared deadline for external operations. */
const rootTestFiles = everyTestFile.filter(
  (path) => path.startsWith("tests/integration/") || path.startsWith("tests/e2e/"),
);
...
test("only the rig's own file reaches the fake editor", () => {
  expect(rigReachers).toEqual(["tests/e2e/editor-death.test.ts"]);
});
...
test("no member's own test spawns, which is why the sweep stops at the root", () => {
  ...
  expect(spawnsIn(readFileSync(join(repoRoot, "tests", "e2e", "cli.test.ts"), "utf8"))).not.toEqual(
    [],
  );
});
