...
function memberSplitAcrossTwoConfigs(extra: Record<string, string> = {}): Record<string, string> {
  return {
    ...
    "packages/late/tests/thing.ts": typeChecks,
    ...extra,
  };
}
...
test("a file covered only by a referenced project is reported WITH the reference named", async () => {
  ...
  // BOTH REPAIRS, BECAUSE THE RULE HAS TWO HALVES: a program counts only while
  // it is TRACKED and named, so a reference to an untracked `lib/tsconfig.json`
  // fires this same sentence -- and a reader told to rename it would be renaming
  // a file whose name is already right.
