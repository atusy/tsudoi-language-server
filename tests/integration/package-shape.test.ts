 ...
 * tests/integration/unbuilt-artifact.test.ts stages those states directly.
 *
 ...
 * entering through RELATIVE imports from this suite's own `tests/*.test.ts`, so
 ...
 * in tests/integration/workspace-members.test.ts against workspaces built for it, since this
 ...
 * things this suite makes: the pack stage in tests/helpers/install.ts and the
 * README checkout in tests/helpers/readme.ts each write package.json into a temp
 ...
 *   tests/e2e/installed-runtime.test.ts, with the pair that drops it.
 ...
 * which tests/integration/published-specifier.test.ts asserts.
 *
...
// tests/e2e/installed-runtime.test.ts, where deno completes the handshake in a
