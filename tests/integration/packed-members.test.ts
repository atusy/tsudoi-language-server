 ...
 * tests/e2e/published-artifacts.test.ts, which stays green through the leak. The
 ...
 * left unread: tests/e2e/published-artifacts.test.ts packs it and installs it into a
...
const readmeTokens: Record<string, readonly RegExp[]> = {
  "@atusy/tsudoi-completion-dictionary": [
    ...
    /default prefix filter uses an in-memory binary search/i,
  ],
  "@atusy/tsudoi-completion-path": [
    /textDocument\/completion/,
  ...
  "@atusy/tsudoi-completion-shell": [
    ...
    /TypeScript consumer needs the peer/,
    /useful only inside a tsudoi host/,
    /Requests to one handler are serialized/i,
  ],
  "@atusy/tsudoi-hover-wordnet": [
    /textDocument\/hover/,
    /peer/i,
 ...
 * `tests/integration/package-shape.test.ts` compiles straight into dist/ and lands in a
...
test("the pattern that found nothing in the tarballs finds the claims in real prose", () => {
  ...
  expect(unreachableClaims(root, source)).toContain("tests/integration/build-order.test.ts");
  expect(
    unreachableClaims(root, "see tests/integration/package-shape.test.ts for the reason"),
  ).toEqual(["tests/integration/package-shape.test.ts"]);
 ...
 * member, on the same reasoning the workspace type check reads
