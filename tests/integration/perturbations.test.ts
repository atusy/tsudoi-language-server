 ...
 *
 * OUTSIDE THE REPOSITORY, for the reason tests/integration/suite-deadline.test.ts already
 * records: a tree under tests/ would be swept by this suite's own enumerations
...
test("a record naming an arm in a file that RE-RUNS perturbations is refused, never spawned", async () => {
  ...
  const record: PerturbationRecord = {
    arm: {
      file: "tests/integration/perturbations.test.ts",
...
const memberArms = "tests/integration/tsudoi-completion-path/resolve.test.ts";
...
const records: readonly PerturbationRecord[] = [
  {
    ...
    arm: {
      file: "tests/integration/artifact-detector.test.ts",
  ...
  },
  {
    ...
    // groups.
    arm: {
      file: "tests/integration/tsudoi-completion-path/resolve.test.ts",
  ...
  },
  {
    ...
    arm: {
      file: "tests/integration/readme-layout.test.ts",
      name: "the layout holds over docs/README.md block 1",
    },
    weakening: {
      file: "tests/helpers/readme.ts",
  ...
  },
  {
    ...
    // from the set below, which `read` compares exactly. What is enforced by
    // nothing is that arm's EXISTENCE: delete it and this record still reports
  ...
  },
  {
    ...
    // It stays green because it reads `detail` alone. No other arm in that file
    // reads a label on a name the flattening rewrites.
    alsoReddens: [],
  },
  {
  ...
  },
  {
    ...
    arm: {
      file: "tests/e2e/progress-token-range.test.ts",
  ...
  },
  {
    ...
    arm: {
      file: "tests/integration/custom-method-types.test.ts",
  ...
  },
  {
    ...
    arm: {
      file: "tests/e2e/custom-notification.test.ts",
  ...
  },
  {
    ...
    arm: {
      file: "tests/e2e/custom-notification-budget.test.ts",
