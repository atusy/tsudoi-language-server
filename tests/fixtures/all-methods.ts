 ...
 *
 * IT EXISTS FOR THE BY-CONSTRUCTION TESTS IN tests/e2e/methods-table.test.ts, which
 ...
 * not resolve` in tests/integration/unbuilt-checkout.test.ts, which runs `tsc` over a staged
 ...
 * which drives tests/fixtures/no-methods.ts. A method joining the table joins
...
export default (): Promise<TsudoiConfig> => {
  return Promise.resolve({
    methods: {
      ...
      // tests/fixtures/execute-command-echo.ts instead, which is where an author
      // deciding it belongs.
      ...
      // action MEANS is asserted against tests/fixtures/code-action.ts instead,
      // which is the file an author deciding it belongs in.
      //
      // AND IT COSTS NO `require-yield` WARNING, MEASURED RATHER THAN ASSUMED,
      // because the claim at tests/fixtures/throws-on-cancel.ts depends on it:
