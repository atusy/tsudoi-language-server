 ...
 * tests/e2e/installed-runtime.test.ts drives the SAME consumer layout and the SAME
 ...
 * NOTHING under src/, examples/, tests/ or scripts/, and it is unpublished, so it
...
test("an installed consumer answers a completion and then resolves one of its own items", async () => {
  ...
  try {
    ...
    try {
      ...
      const answer = await running.request<CompletionList>("textDocument/completion", {
        textDocument: { uri: documentUri },
