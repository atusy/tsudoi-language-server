 ...
 * chosen:
 *
 * THE FLOOR IS 20_000, `handshakeTimeoutMs` in tests/helpers/readme.ts, which is
 ...
 * tests/e2e/protocol.test.ts crossed 4000ms. So the work in front of the handshake
 ...
 * fake editor's own self-exit (tests/helpers/fake-editor.ts), so the two
 * deadlines would coincide; that timer is reachable only from the two rig tests
 * in tests/e2e/editor-death.test.ts, which set 20_000 for themselves and so fire
 ...
 * HERE makes the over-arms in tests/integration/suite-deadline.test.ts pass, so every run of
 ...
 * THE TWO APART. Every arm in tests/integration/suite-deadline.test.ts pins the variable in
 ...
 *
 * ITS OWN MODULE AND NOT tests/helpers/build.ts: the build throws on a failed
 * compile, and a
 ...
 * tests/integration/suite-deadline.test.ts rather than by care. MEASURED that the hole is
