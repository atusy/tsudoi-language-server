...
description: Use when taking a measurement, reporting an exit code or a test result, writing a requirement or a plan, or stating a count, a coverage claim, or a fact about how something in this repository behaves. Carries the measured ways a stated fact here turned out to be unmeasured.
---

# Recording a measurement, and authoring what binds on one

**WHY THIS SKILL EXISTS, MEASURED RATHER THAN ASSUMED**: criteria
...
**A CRITERION THAT WENT UNMET IS RECORDED AS UNMET.** The value of
...
happened — the maintainer removed the thing it named, and the author recorded
...
refusing it is why the sign-off could be given.

## Taking the reading

**A command whose exit code is being reported is run UNPIPED, and the report
carries the COMMAND AS RUN, not only its exit.** THIRD OCCURRENCE
OF ONE CLASS across two people, one of them long-standing, and the
...
**Run a verification command exactly as `scripts/definition-of-done.json`
spells it, and trust a wrapper only after you have seen it redden once.**
...
names the one it ran on.** The suite runs both runtimes precisely
...
A 0/0/0 reading with counts unchanged reads as CONFIRMATION
...
POISONED MEASUREMENT. MEASURED: `tsc` WRITES `dist/` AND THEN EXITS
...
**`whose cost, at what size, on which runtime` STOOD HERE AS THE
...
the claim grew across three re-authorings before review measured it. MEASURED, and the
...
A cancellation checkpoint was refused on a complete measurement
...
without checking it against the remedy it justifies.** **A measured
...
original could not, AND THAT DIFFERENCE IS WHY THE ERROR SURVIVED.

**A labelled claim is measured while it is still cheap.** MEASURED:
...
NOT take the "reasoned" option the label rule allows.** Widened
...
the same session.** Three instances in one round of planning; two of them
were claims about a test in this repository, made while invoking that same test's
authority.

**Count the CLAIM'S SUBJECT, not one mechanism's call sites.**
...
**Re-measure a number you were handed rather than copying it.** This
has caught a handed-down count twice in a row, and it works BECAUSE
...
**A handoff carries its provenance at both ends.** One rule, three
...
**A list of which arms reddened is a reading OF A SPELLING, not of
a perturbation's description. Cited without the spelling it can be neither
reconciled nor re-run.** MEASURED twice in one change, from opposite ends:
...
- Three degenerates described identically but spelled WIDER — the shared reader
  ...
  leaves alone.
- A reviewer and an author ran "the same" transform and produced **different
...
failure that reads as courtesy.** An author was handed a
...
An edit to the measurement; an edit to the file it describes; a
LATER CHANGE ALTERING THE WORLD IT MEASURED — no grep finds this, because the
...
KNOWN PRIOR READING.** And the half that is not about the verdict:
...
and never its own fix.** MEASURED: a criterion whose
...
author followed the remedy and the property went unmet, with the record then
claiming it met.
...
The disagreement is real information and the temptation is to
...
because it made the perturbation cheap; the maintainer had been shown both
...
criterion amended so a failing change passes is argument; one amended AFTER
the change was rebuilt to the stricter reading is a repaired instrument. Same
...
MEASURED: a control was written as "the two NOT COMPLETE
...
visibly a guess.** MEASURED: a literal resolution probe against
...
unmeetable criterion.** "Staleness must be impossible" was
...
**At a mechanism change, ask which constraints SURVIVE it.** Those
...
the first.** The value of writing criteria at all is that "this one
...
A key that fails to MATCH stops applying. Eight tracked
...
**How each is held — the sentence here said all four
...
**MEASURED, and the cost was the whole finding.** A Definition-of-
